...
| **`float16` conversion execution on the oldest gated GCC** — `unity-f16-exec-gcc-floor.yml` | the `float16` kernels that widen or narrow between half and single precision produce correct results when built by the floor of the supported GCC range, not merely compile and link. That range is supported down to GCC 13 only because the library hand-encodes those MVE conversions — the assembler shipped with the older releases encodes them incorrectly, which a build-and-link gate cannot see and which faults or corrupts registers at run time ([#427](https://github.com/AmbiqAI/ns-cmsis-nn/issues/427)) | cortex-m55 under QEMU's `mps3-an547` machine, the same AN547 board the Corstone-300 FVP models, built at the release leg's own `-Ofast` with `ARM_NN_ENABLE_F32` and `ARM_NN_ENABLE_F16`. The toolchain is pinned by URL and SHA-256 in `ci/toolchains/arm-gnu-floor.json`, and the job fails if that pin is no longer the oldest release in `toolchain-matrix-strict-link.yml`'s matrix or if the downloaded compiler's own banner disagrees with it. Scope is the suites whose kernels reach the conversion sites, not the whole `float16` set |
...
not a qualification vehicle: it is there because the defect that leg guards
against is introduced by the assembler and so needs a *second compiler* to
be executed at all, which the FVP legs — all on the container's single
