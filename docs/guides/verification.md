...
| **Toolchain build + strict link** | every kernel compiles and every symbol resolves — no `--gc-sections`, no ignored undefined symbols | GCC 13.2.Rel1 / 14.2.Rel1 / 15.3.Rel1, ATfE 19.1.5 and armclang 6.23.32, each on cortex-m55 and cortex-m4 (the armclang cell needs a license secret, so it skips on fork PRs). `float16` is built only from the GCC float16 floor up, declared as `GCC_F16_FLOOR` in `toolchain-matrix-strict-link.yml`; a further cell builds on a release below that floor and fails unless the configure refuses `float16` with remediation text and the documented escape hatch still configures and warns |
| **`float16` conversion execution on the float16 toolchain floor** — `unity-f16-exec-gcc-floor.yml` | the `float16` kernels that widen or narrow between half and single precision produce correct results when built by the oldest GCC release allowed to build them, not merely compile and link. Those conversions are encoded by the assembler rather than the compiler, so nothing short of executing sees a wrong register or an UNDEFINED word ([#427](https://github.com/AmbiqAI/ns-cmsis-nn/issues/427)) | cortex-m55 under QEMU's `mps3-an547` machine, the same AN547 board the Corstone-300 FVP models, built at the release leg's own `-Ofast` with `ARM_NN_ENABLE_F32` and `ARM_NN_ENABLE_F16`. The toolchain is pinned by URL and SHA-256 in `ci/toolchains/arm-gnu-floor.json`, and the job fails if that pin is not the `GCC_F16_FLOOR` declared in `toolchain-matrix-strict-link.yml`, if that floor is not itself a gated release, or if the downloaded compiler's own banner disagrees with it. Scope is the legacy Unity suites for the kernels that use a Q-form half/single conversion intrinsic, hand-listed and cross-checked against a list derived from the tree, so the job fails if the list is short of it, not the whole `float16` set |
| **Memory safety** — host sanitizer | out-of-bounds access, undefined behavior and leaks that leave the numerics intact, such as a scratch buffer under-sized by a `get_buffer_size` query | **x86 host**, scalar (non-MVE) paths; the Unity suites `ctest` registers, run under ASan + UBSan + LSan. The job reads the registered-suite count back from `ctest -N` and fails below a floor, and runs with `--no-tests=error`, so a suite that quietly stops being built is a failure rather than a smaller green run |
| **Legacy Unity compile gate** — `unity-m55-compile.yml` | every legacy Unity test translation unit compiles and links with the release leg's own flags and `-Werror`, so a test file cannot first meet a cross-compiler at release time; it does not execute anything under the FVP | cortex-m55, cortex-m4 and cortex-m0 — `legacy-tester.yml`'s own matrix — each built in the CI container with the harness's pinned Arm GNU toolchain and that leg's float flags (`ARM_NN_ENABLE_F32` on all three, `ARM_NN_ENABLE_F16` on m55 only, the exact per-CPU suite set is asserted by the workflow itself — declared, built and on-disk sets must match by name — so the authoritative counts live in each run's log, not in this guide). The m4 cell is what compiles the `ARM_MATH_DSP && !ARM_MATH_MVEI` blocks in 12 Unity test files, which an m55 build never reaches. The m0 cell is not there for a preprocessor shape — the host-sanitizer row directly above already compiles the neither-macro shape of the same files on x86 under `-Werror`, with a smaller warning set than the target cells — but for the target and toolchain: arm-none-eabi 14.2.rel1 at `-Ofast` on a soft-float, no-FPU core (`-mfloat-abi=soft`). Before compiling, each cell parses the full release token out of the harness's own `GCC_URL` (`Tests/UnitTest/build_and_run_tests.sh`, today `14.2.rel1`) and fails unless the downloaded compiler's `--version` banner reports the same token, compared case-insensitively and in full rather than by major.minor. Each cell also asserts, per CPU, that the suites CMakeLists.txt declares under its options, the test objects the build produced and the suite directories on disk are the same set by name |
