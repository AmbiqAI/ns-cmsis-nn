...
| **Legacy Unity compile gate** — `unity-m55-compile.yml` | every legacy Unity test translation unit compiles and links with the release leg's own flags and `-Werror`, so a test file cannot first meet a cross-compiler at release time; it does not execute anything under the FVP | cortex-m55 only, built in the CI container with the harness's pinned Arm GNU toolchain (`Tests/UnitTest/build_and_run_tests.sh`, whose pinned version the job asserts before it compiles). The cortex-m0/m4-only `ARM_MATH_DSP` paths stay out of scope and are still first compiled at nightly/release: 12 Unity test files carry `ARM_MATH_DSP && !ARM_MATH_MVEI` blocks that an m55 build never reaches |
