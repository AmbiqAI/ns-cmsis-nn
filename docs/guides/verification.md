...
| **`float16` conversion execution on the oldest gated GCC** — `unity-f16-exec-gcc-floor.yml` | the `float16` kernels that widen or narrow between half and single precision produce correct results when built by the floor of the supported GCC range, not merely compile and link — the defect this catches is introduced by the assembler, so nothing short of executing on a second compiler sees it ([#427](https://github.com/AmbiqAI/ns-cmsis-nn/issues/427)) | cortex-m55 under QEMU's `mps3-an547` machine, the same AN547 board the Corstone-300 FVP models, built at the release leg's own `-Ofast` with `ARM_NN_ENABLE_F32` and `ARM_NN_ENABLE_F16`. The toolchain is pinned by URL and SHA-256 in `ci/toolchains/arm-gnu-floor.json`, and the job fails if that pin is no longer the oldest release in `toolchain-matrix-strict-link.yml`'s matrix or if the downloaded compiler's own banner disagrees with it. Scope is the legacy Unity suites whose kernels reach the conversion sites, not the whole `float16` set |
