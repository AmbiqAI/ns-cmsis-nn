...
| **Legacy Unity compile gate** — `unity-m55-compile.yml` | every legacy Unity test translation unit compiles and links with the release leg's own flags and `-Werror`, so a test file cannot first meet a cross-compiler at release time; it does not execute anything under the FVP | cortex-m55 only, built in the CI container with the harness's pinned Arm GNU toolchain. Before compiling, the job parses the full release token out of that harness's own `GCC_URL` (`Tests/UnitTest/build_and_run_tests.sh`, today `14.2.rel1`) and fails unless the downloaded compiler's `--version` banner reports the same token, compared case-insensitively and in full rather than by major.minor. The cortex-m0/m4-only `ARM_MATH_DSP` paths stay out of scope and are still first compiled at nightly/release: 12 Unity test files carry `ARM_MATH_DSP && !ARM_MATH_MVEI` blocks that an m55 build never reaches |
...
fails if a required asset is missing. The Unity suites' cortex-m55 *build*
now also runs on pull requests, through `unity-m55-compile.yml` in the table
above; their *execution*, and the cortex-m0 and cortex-m4 cells, still run
only here and nightly. `release-verify` does not run on pull requests at all.
