...
```cmake
list(APPEND CMAKE_PREFIX_PATH "${CMAKE_SOURCE_DIR}/third_party/ns-cmsis-nn-cortex-m4-atfe-7.31.0") # x-release-please-version

find_package(ns-cmsis-nn 7.31.0 REQUIRED CONFIG) # x-release-please-version
...
```bash
cmake -S . -B build \
  -DCMAKE_PREFIX_PATH="$PWD/third_party/ns-cmsis-nn-cortex-m4-atfe-7.31.0" # x-release-please-version
