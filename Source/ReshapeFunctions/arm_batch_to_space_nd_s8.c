...
{
    // Validate input
    if (!input_data || !input_dims || !block_shape || !crop || !output_data || !output_dims)
    {
    ...
    if (output_dims->n != ON || output_dims->h != OH || output_dims->w != OW || output_dims->c != OC)
    {
        return ARM_CMSIS_NN_ARG_ERROR;
    }
