                                             ...
                                             const int32_t output_offset)
{
    ...
    if (output_dims)
    {
        if (output_dims->n != Bout || output_dims->h != Hout || output_dims->w != Wout || output_dims->c != C)
        {
            return ARM_CMSIS_NN_ARG_ERROR;
        }
    }
    ...
    const uint32_t out_bytes = (uint32_t)Bout * (uint32_t)Hout * (uint32_t)Wout * (uint32_t)C;
