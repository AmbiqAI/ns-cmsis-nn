...
arm_cmsis_nn_status arm_logistic_s16(
  ...
  while (count > 0) {
    ...
    int32x4_t temp_lo = vaddq_n_s32(vmulq_n_s32(vec_in_lo,  input_multiplier), round);
    int32x4_t scaled_lo = vshlq_s32(temp_lo, vdupq_n_s32(-input_left_shift));
    int32x4_t temp_hi = vaddq_n_s32(vmulq_n_s32(vec_in_hi, input_multiplier), round);
    int32x4_t scaled_hi = vshlq_s32(temp_hi, vdupq_n_s32(-input_left_shift));
    ...
    uh = vrshrnbq_n_u32(uh, abs_lo, 9);
    uh = vrshrntq_n_u32(uh, abs_hi, 9);
    ...
    uint32x4_t interp_lo = vshlq_n_u32(ua_lo, 9) + vmulq_u32(ut_lo, vsubq_u32(ub_lo, ua_lo));
    uint32x4_t interp_hi = vshlq_n_u32(ua_hi, 9) + vmulq_u32(ut_hi, vsubq_u32(ub_hi, ua_hi));

    /* For values with index (uh) >= 255, saturate to maximum: 0x7FFF << 10 */
    uint32x4_t sat_val = vdupq_n_u32(0x7FFF << 10);
    interp_lo = vpselq_u32(interp_lo, sat_val, p_sat);
    interp_hi = vpselq_u32(interp_hi, sat_val, p_sat);
    ...
    */
    ...
    int32x4_t pos_res_lo = vaddq_n_s32(vreinterpretq_s32_u32(interp_lo), 1 << 9);
    int32x4_t pos_res_hi = vaddq_n_s32(vreinterpretq_s32_u32(interp_hi), 1 << 9);

    int32x4_t neg_base = vdupq_n_s32(((1 << (16 + 9)) + (1 << 9) - 1));
    int32x4_t neg_res_lo = vsubq_s32(neg_base, vreinterpretq_s32_u32(interp_lo));
    int32x4_t neg_res_hi = vsubq_s32(neg_base, vreinterpretq_s32_u32(interp_hi));

    int32x4_t final_lo = vpselq_s32(pos_res_lo, neg_res_lo, p_pos_lo);
    int32x4_t final_hi = vpselq_s32(pos_res_hi, neg_res_hi, p_pos_hi);

    /* Shift right by 10 to convert back to 16-bit range. */
    int16x8_t vec_result = vdupq_n_s16(0);
    vec_result = vrshrnbq_n_s32(vec_result, final_lo, 10);
    vec_result = vrshrntq_n_s32(vec_result, final_hi, 10);

    /* Store the results using a masked store */
    vst1q_p_s16(output, vec_result, p);
