                                        ...
                                        int8_t *output)
{
    ...
    // A caller that reports its allocation gets it checked against what the kernel sums below actually index.
    // ctx->size == 0 means "not reported" and skips the check, which is what the in-tree tests pass.
    if (ctx->size > 0 && ctx->size < arm_batch_matmul_s8_get_buffer_size(input_rhs_dims))
    {
        return ARM_CMSIS_NN_ARG_ERROR;
    }
