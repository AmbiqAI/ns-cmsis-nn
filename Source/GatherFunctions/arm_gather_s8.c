 ...
 * Title:        arm_gather_s8.c
 * Description:  Gather operator implementations for s8 tensors.
 *
                                  ...
                                  const cmsis_nn_dims *output_dims)
{
    ...
    {
        const int32_t dim = arm_cmsis_nn_dim_at(input_dims, i);
        if (dim < 0)
        {
            return ARM_CMSIS_NN_ARG_ERROR;
        }
    ...
    size_t input_total = arm_cmsis_nn_shape_product(input_shape, params->input_rank);
    ...
    size_t output_dims_total = arm_cmsis_nn_shape_product(expected_shape, expected_rank);
    ...
    for (size_t batch = 0; batch < batch_size; ++batch)
    {
        ...
        for (size_t outer = 0; outer < outer_size; ++outer)
        {
            ...
            for (size_t coord = 0; coord < coord_size; ++coord)
            {
                ...
                arm_memcpy_s8(output_data + dst_offset, input_data + src_offset, (uint32_t)inner_size);
            }
        }
    }
