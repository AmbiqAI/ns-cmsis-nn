 ...
 * enters. On the reset-after path (reset_after != 0) the reset gate is folded per hidden unit in automatics,
 * temp1 is never dereferenced on any build path, and both the wrapper and the step accept it as NULL - so the
 * requirement is zero there.
 */
