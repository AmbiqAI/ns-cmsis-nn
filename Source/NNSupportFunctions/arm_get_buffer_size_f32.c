                                               ...
                                               arm_nn_tensor_layout layout)
{
        ...
        output_dims->c >= CONVERT_DW_CONV_WITH_ONE_INPUT_CH_AND_OUTPUT_CH_ABOVE_THRESHOLD)
    {
        ...
        const int32_t conv_buf_size =
            arm_convolve_wrapper_f32_get_buffer_size(&conv_params, input_dims, &conv_filter_dims, output_dims);
        size_t filter_bytes = (size_t)ARM_NN_ROUND_UP(output_dims->c, 4);
        ...
        if (!arm_nn_checked_size_mul(filter_bytes, (size_t)filter_dims->h, &filter_bytes) ||
            ...
            !arm_nn_checked_size_mul(filter_bytes, sizeof(float32_t), &filter_bytes))
        {
            return 0;
        }
...
int32_t arm_depthwise_conv_wrapper_f32_get_buffer_size(const cmsis_nn_dw_conv_params_f32 *dw_conv_params,
                                                       ...
                                                       const cmsis_nn_dims *output_dims)
{
    return arm_depthwise_conv_f32_get_buffer_size(
                                         ...
                                         arm_nn_tensor_layout layout)
{
    ...
    /* Grouped convolution uses the scratch-free generic path. */
    if (groups > 1)
    {
        return 0;
    }
    ...
    size_t tile_bytes = (size_t)ARM_NN_CONV_NHWC_PATCH_GEMM_F32_MAX_TILE_ROWS;
    ...
    if (!arm_nn_checked_size_mul(tile_bytes, (size_t)filter_dims->h, &tile_bytes) ||
        ...
        !arm_nn_checked_size_mul(tile_bytes, sizeof(float32_t), &tile_bytes))
    {
        return 0;
    }
...
int32_t arm_convolve_wrapper_f32_get_buffer_size(const cmsis_nn_conv_params_f32 *conv_params,
                                                 ...
                                                 const cmsis_nn_dims *output_dims)
{
    return arm_convolve_f32_get_buffer_size(conv_params, input_dims, filter_dims, output_dims, ARM_NN_LAYOUT_NHWC);
}
                                               ...
                                               arm_nn_tensor_layout layout)
{
    ...
    if (!arm_nn_checked_size_mul(row_bytes, (size_t)input_dims->c, &row_bytes) ||
        !arm_nn_checked_size_mul(row_bytes, sizeof(float32_t), &row_bytes))
    {
        return 0;
    }
...
int32_t arm_fully_connected_f32_get_buffer_size(const cmsis_nn_fc_params_f32 *fc_params,
                                                ...
                                                arm_nn_tensor_layout layout)
{
    /* Current float32 fully-connected implementations are bufferless. */
    (void)fc_params;
    (void)input_dims;
    (void)filter_dims;
    (void)output_dims;
...
int32_t arm_batch_matmul_f32_get_buffer_size(const cmsis_nn_bmm_params_f32 *bmm_params,
                                             ...
                                             const cmsis_nn_dims *output_dims)
{
    /* Current float32 batch-matmul implementations are bufferless. */
    (void)bmm_params;
    (void)input_lhs_dims;
    (void)input_rhs_dims;
    (void)output_dims;
