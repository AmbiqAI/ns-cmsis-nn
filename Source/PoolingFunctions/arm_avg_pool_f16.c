 ...
 * Title:        arm_avg_pool_f16.c
 * Description:  Avg pooling function implementations (float16)
 *
 * $Date:        26 Feb 2026
 * $Revision:    V.1.0.0
 *
                                     ...
                                     float16_t *dst)
{
    ...
    const float16_t act_min = pool_params->activation.min;
    const float16_t act_max = pool_params->activation.max;
    ...
    while (batch_cnt)
    {
        for (int32_t i_y = 0, base_idx_y = -pad_y; i_y < output_y; base_idx_y += stride_y, i_y++)
        {
            for (int32_t i_x = 0, base_idx_x = -pad_x; i_x < output_x; base_idx_x += stride_x, i_x++)
            {
                ...
                const float16_t inv_count = (count > 0) ? (float16_t)(1.0f / (float32_t)count) : (float16_t)0.0f;

                for (int32_t c = 0; c < channel_in; c += 8)
                {
                    const mve_pred16_t p = vctp16q((uint32_t)(channel_in - c));
                    float16x8_t v_sum = vdupq_n_f16((float16_t)0.0f);
                    ...
                    float16x8_t v_avg = vmulq(v_sum, inv_count);
                    v_avg = arm_nn_clamp_mve_f16(v_avg, v_act_min, v_act_max);
                    vst1q_p(dst + (i_y * output_x + i_x) * channel_in + c, v_avg, p);
                }
    #else
                for (int32_t c = 0; c < channel_in; ++c)
                {
                    ...
                    for (int32_t k_y = ker_y_start; k_y < kernel_y_end; k_y++)
                    {
                        ...
                        {
                            const int32_t idx = (k_x + base_idx_x + (k_y + base_idx_y) * input_x) * channel_in + c;
                            sum += (float32_t)src[idx];
                        }
                    }
                    const float16_t v = (count > 0) ? (float16_t)(sum / (float32_t)count) : (float16_t)0.0f;
                    dst[(i_y * output_x + i_x) * channel_in + c] = arm_nn_clamp_scalar_f16(v, act_min, act_max);
                }
    #endif
            }
        }
...
arm_cmsis_nn_status arm_avg_pool_nhwc_f16(const cmsis_nn_context *ctx,
                                          const cmsis_nn_pool_params_f16 *pool_params,
                                          const cmsis_nn_dims *input_dims,
                                          const float16_t *src,
                                          const cmsis_nn_dims *filter_dims,
                                          ...
                                          float16_t *dst)
{
    return arm_avg_pool_f16(ctx, pool_params, input_dims, src, filter_dims, output_dims, dst);
}
