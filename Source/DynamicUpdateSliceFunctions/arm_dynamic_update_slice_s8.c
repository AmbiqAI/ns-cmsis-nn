 ...
 * Title:        arm_dynamic_update_slice_s8.c
 * Description:  DynamicUpdateSlice operator for s8 tensors.
 *
                                                ...
                                                int8_t *output)
{
    ...
    /* Overwrite the slice with update data using row-level memcpy */
    const int32_t inner_dim = update_shape[rank - 1];
    const int32_t num_rows = update_size / inner_dim;

    for (int32_t row = 0; row < num_rows; row++)
    {
        /* Compute multi-dim coordinates of this row in update space */
        ...
        memcpy(output + out_offset, update + row * inner_dim,
               (size_t)inner_dim * sizeof(int8_t));
    }
