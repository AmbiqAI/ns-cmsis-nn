...
static inline bool stride_continue_f16(int32_t idx, int32_t stop, int32_t stride)
{
                                          ...
                                          const cmsis_nn_dims *const output_dims)
{
        ...
        b_c == 0 && o_c == in_c)
    {
        ...
        arm_memcpy_f16(output_data, input_data + offset, total);
    ...
    if (s_h == 1 && s_w == 1 && s_c == 1 && b_w == 0 && o_w == in_w && b_c == 0 && o_c == in_c)
    {
        ...
        for (int32_t n = 0; n < o_n; n++)
        {
            ...
            arm_memcpy_f16(output_data + n * batch_block, input_data + start, batch_block);
        }
        return ARM_CMSIS_NN_SUCCESS;
    }
    ...
    if (s_w == 1 && s_c == 1 && b_c == 0 && o_c == in_c)
    {
        ...
        for (int32_t n = 0; n < o_n; n++)
        {
            ...
            for (int32_t h = 0; h < o_h; h++)
            {
                ...
                int32_t start = base_n + (b_h + h * s_h) * slice_elems + b_w * in_c; // row_elems == in_c

                // copy only o_w*in_c elements (i.e. the slice’s columns)
                arm_memcpy_f16(output_data + out_idx, input_data + start, row_block);
                out_idx += row_block;
            }
        }
        return ARM_CMSIS_NN_SUCCESS;
        ...
        for (int32_t n = 0; n < o_n; n++)
        {
            ...
            for (int32_t h = 0; h < o_h; h++)
            {
                ...
                for (int32_t w = 0; w < o_w; w++)
                {
                    ...
                    arm_memcpy_f16(output_data + out_idx, input_data + start, o_c);
                    out_idx += o_c;
                }
            }
        }
    ...
    // General case: truly arbitrary strides (including negative)
    {
        ...
        int32_t out_idx = 0;
        for (int32_t off0 = b_n; stride_continue_f16(off0, stop0, s_n); off0 += s_n)
        {
            for (int32_t off1 = b_h; stride_continue_f16(off1, stop1, s_h); off1 += s_h)
            {
                for (int32_t off2 = b_w; stride_continue_f16(off2, stop2, s_w); off2 += s_w)
                {
                    for (int32_t off3 = b_c; stride_continue_f16(off3, stop3, s_c); off3 += s_c)
                    {
