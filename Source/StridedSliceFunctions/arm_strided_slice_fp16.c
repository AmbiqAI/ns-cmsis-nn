 ...
 * Title:        arm_strided_slice_fp16.c
 * Description:  StridedSlice function for fp16 data compatible with TF Lite.
 *
 * $Date:        22 July 2026
 * $Revision:    V.1.0.0
 *
...
static inline bool stride_continue_fp16(int32_t idx, int32_t stop, int32_t stride)
{
...
/*
 * fp16 strided slice function for TensorFlow Lite
 *
 * StridedSlice is a pure data-movement op, so the s16 copy is bit-exact for
 * float16 payloads (both are 2-byte elements).
 *
                                           ...
                                           const cmsis_nn_dims *const  output_dims)
{
     ...
     && b_c==0     && o_c==in_c)
    {
        ...
        arm_memcpy_s16((int16_t *)output_data,
                      (const int16_t *)(input_data + offset),
                      total);
     ...
     && b_c == 0 && o_c == in_c)
    {
        ...
        for (int32_t n = 0; n < o_n; n++)
        {
            ...
            int32_t start = (b_n + n * s_n) * plane_elems
                          +  b_h            * slice_elems;
            arm_memcpy_s16(
              (int16_t *)(output_data + n * batch_block),
              (const int16_t *)(input_data  + start),
              batch_block
            );
        }
        return ARM_CMSIS_NN_SUCCESS;
            ...
            for (int32_t h = 0; h < o_h; h++)
            {
                ...
                for (int32_t w = 0; w < o_w; w++)
                {
                    int32_t start = base_h
                                  + (b_w + w*s_w)*row_elems
                                  + b_c;
                    arm_memcpy_s16((int16_t *)(output_data + out_idx),
                                  (const int16_t *)(input_data  + start),
                                  o_c);
                    out_idx += o_c;
                }
    ...
    // General case: truly arbitrary strides (including negative)
    {
        ...
        int32_t out_idx = 0;
        for (int32_t off0 = b_n; stride_continue_fp16(off0, stop0, s_n); off0 += s_n)
        {
            for (int32_t off1 = b_h; stride_continue_fp16(off1, stop1, s_h); off1 += s_h)
            {
                for (int32_t off2 = b_w; stride_continue_fp16(off2, stop2, s_w); off2 += s_w)
                {
                    for (int32_t off3 = b_c; stride_continue_fp16(off3, stop3, s_c); off3 += s_c)
                    {
                        const int32_t idx =
                            off0 * plane_elems +
                            off1 * slice_elems +
