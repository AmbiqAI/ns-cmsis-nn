 ...
 * Title:        arm_elementwise_sub_broadcast_f16.c
 * Description:  Elementwise subtract for float16 tensors with NHWC broadcasting
 *
...
/* vec - scalar: the arithmetic and clamp of arm_elementwise_sub_f16 with the second operand
 * held in a register, so the result is bit-identical to the flat kernel over a materialised
 * broadcast operand (#415). */
static void arm_elementwise_sub_scalar_f16(const float16_t *vec,
                                           float16_t scalar,
                                           float16_t *output,
                                           ...
                                           int32_t n)
{
    ...
    for (; i <= n - 8; i += 8)
    {
        ...
        vst1q(&output[i], arm_nn_clamp_propagate_nan_mve_f16(vsubq(va, vs), vmin, vmax));
    }
    if (i < n)
    {
        const mve_pred16_t p = vctp16q((uint32_t)(n - i));
        const float16x8_t va = vld1q_z(&vec[i], p);
        vstrhq_p(&output[i], arm_nn_clamp_propagate_nan_mve_f16(vsubq(va, vs), vmin, vmax), p);
    }
    #else
...
/* scalar - vec, computed as written: -(vec - scalar) differs at signed zero (#415). */
static void arm_elementwise_rsub_scalar_f16(const float16_t *vec,
                                            float16_t scalar,
                                            float16_t *output,
                                            float16_t out_activation_min,
                                            float16_t out_activation_max,
                                            int32_t n)
{
    ...
    {
        const float16x8_t va = vld1q(&vec[i]);
        vst1q(&output[i], arm_nn_clamp_propagate_nan_mve_f16(vsubq(vs, va), vmin, vmax));
    }
    if (i < n)
    {
        const mve_pred16_t p = vctp16q((uint32_t)(n - i));
        const float16x8_t va = vld1q_z(&vec[i], p);
        vstrhq_p(&output[i], arm_nn_clamp_propagate_nan_mve_f16(vsubq(vs, va), vmin, vmax), p);
    }
    #else
    for (int32_t i = 0; i < n; ++i)
    {
        const _Float16 v = (_Float16)scalar - (_Float16)vec[i];
        output[i] = arm_nn_clamp_scalar_f16((float16_t)v, out_activation_min, out_activation_max);
    }
    #endif
}
    ...
    #define ARM_SUB_BCAST_F16_FULL(a, b, o, n)                                                                         \
        (void)arm_elementwise_sub_f16((a), (b), (o), out_activation_min, out_activation_max, (n))
    #define ARM_SUB_BCAST_F16_SCALAR_1(s, v, o, n)                                                                     \
        arm_elementwise_rsub_scalar_f16((v), *(s), (o), out_activation_min, out_activation_max, (n))
    #define ARM_SUB_BCAST_F16_SCALAR_2(s, v, o, n)                                                                     \
        arm_elementwise_sub_scalar_f16((v), *(s), (o), out_activation_min, out_activation_max, (n))

/*
 * float16 elementwise subtract with NHWC broadcasting.
 *
                                                      ...
                                                      float16_t out_activation_max)
{
    ...
    ARM_NN_BROADCAST_WALK_NHWC(float16_t,
                               ...
                               output_data,
                               output_dims,
                               ARM_SUB_BCAST_F16_FULL,
                               ARM_SUB_BCAST_F16_SCALAR_1,
                               ARM_SUB_BCAST_F16_SCALAR_2);

    return ARM_CMSIS_NN_SUCCESS;
}

    #undef ARM_SUB_BCAST_F16_FULL
    #undef ARM_SUB_BCAST_F16_SCALAR_1
    #undef ARM_SUB_BCAST_F16_SCALAR_2
