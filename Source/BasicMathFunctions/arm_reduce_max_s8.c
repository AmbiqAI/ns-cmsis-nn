 ...
 * @return     ARM_CMSIS_NN_SUCCESS on success
 */
arm_cmsis_nn_status arm_reduce_max_spatial_s8(const int8_t *input_data,
                                              const cmsis_nn_dims *input_dims,
                                              const cmsis_nn_dims *axis_dims,
                                              int8_t *output_data,
                                              const cmsis_nn_dims *output_dims)
{
    ...
    return arm_reduce_max_generic_s8(input_data, input_dims, axis_dims, output_data, output_dims);
                                      ...
                                      const cmsis_nn_dims *output_dims)
{
    ...
    if (suffix_start >= 0)
    {
        ...
        return arm_reduce_max_flatten_last_dims_s8(input_data, output_data, outer_size, inner_size);
    }

    // Check for spatial reduction axis=[H,W]
    if (!axis_dims->n && axis_dims->h && axis_dims->w && !axis_dims->c)
    {
        return arm_reduce_max_spatial_s8(input_data, input_dims, axis_dims, output_data, output_dims);
    }
