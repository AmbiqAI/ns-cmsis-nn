 ...
 * Title:        arm_argmin_s16
 * Description:  ArgMin reduction along a chosen axis for s16 tensors
 *
                                   ...
                                   int32_t *output_data)
{
    ...
    for (int32_t outer = 0; outer < outer_count; ++outer)
    {
        ...
        for (int32_t inner = 0; inner < inner_count; ++inner)
        {
            ...
            const int16_t *ptr = input_data + base_index;

            int32_t best_index = 0;
            int16_t best_value = ptr[0];

            for (int32_t k = 1; k < axis_dim; ++k)
            {
                const int16_t value = ptr[k * inner_count];
                if (value < best_value)
                {
                    best_value = value;
                    best_index = k;
