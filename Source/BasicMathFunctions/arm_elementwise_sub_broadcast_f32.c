 ...
 * Title:        arm_elementwise_sub_broadcast_f32.c
 * Description:  Elementwise subtract for float32 tensors with NHWC broadcasting
 *
...
/* vec - scalar: the arithmetic and clamp of arm_elementwise_sub_f32 with the second operand
 * held in a register, so the result is bit-identical to the flat kernel over a materialised
 * broadcast operand (#415). */
static void arm_elementwise_sub_scalar_f32(const float32_t *vec,
                                           float32_t scalar,
                                           float32_t *output,
                                           ...
                                           int32_t n)
{
    ...
    for (; i <= n - 4; i += 4)
    {
        ...
        vst1q(&output[i], arm_nn_clamp_propagate_nan_mve_f32(vsubq(va, vs), vmin, vmax));
    }
    if (i < n)
    {
        const mve_pred16_t p = vctp32q((uint32_t)(n - i));
        const float32x4_t va = vld1q_z(&vec[i], p);
        vstrwq_p(&output[i], arm_nn_clamp_propagate_nan_mve_f32(vsubq(va, vs), vmin, vmax), p);
    }
    #else
...
static void arm_elementwise_rsub_scalar_f32(const float32_t *vec,
                                            float32_t scalar,
                                            float32_t *output,
                                            float32_t out_activation_min,
                                            float32_t out_activation_max,
                                            int32_t n)
{
    ...
    {
        const float32x4_t va = vld1q(&vec[i]);
        vst1q(&output[i], arm_nn_clamp_propagate_nan_mve_f32(vsubq(vs, va), vmin, vmax));
    }
    if (i < n)
    {
        const mve_pred16_t p = vctp32q((uint32_t)(n - i));
        const float32x4_t va = vld1q_z(&vec[i], p);
        vstrwq_p(&output[i], arm_nn_clamp_propagate_nan_mve_f32(vsubq(vs, va), vmin, vmax), p);
    }
    #else
    for (int32_t i = 0; i < n; ++i)
    {
        const float32_t v = scalar - vec[i];
        output[i] = arm_nn_clamp_scalar_f32(v, out_activation_min, out_activation_max);
    }
    #endif
}
    ...
    #define ARM_SUB_BCAST_F32_FULL(a, b, o, n)                                                                         \
        (void)arm_elementwise_sub_f32((a), (b), (o), out_activation_min, out_activation_max, (n))
    #define ARM_SUB_BCAST_F32_SCALAR_1(s, v, o, n)                                                                     \
        arm_elementwise_rsub_scalar_f32((v), *(s), (o), out_activation_min, out_activation_max, (n))
    #define ARM_SUB_BCAST_F32_SCALAR_2(s, v, o, n)                                                                     \
        arm_elementwise_sub_scalar_f32((v), *(s), (o), out_activation_min, out_activation_max, (n))

/*
 * float32 elementwise subtract with NHWC broadcasting.
 *
                                                      ...
                                                      float32_t out_activation_max)
{
    ...
    ARM_NN_BROADCAST_WALK_NHWC(float32_t,
                               ...
                               output_data,
                               output_dims,
                               ARM_SUB_BCAST_F32_FULL,
                               ARM_SUB_BCAST_F32_SCALAR_1,
                               ARM_SUB_BCAST_F32_SCALAR_2);

    return ARM_CMSIS_NN_SUCCESS;
}

    #undef ARM_SUB_BCAST_F32_FULL
    #undef ARM_SUB_BCAST_F32_SCALAR_1
    #undef ARM_SUB_BCAST_F32_SCALAR_2
