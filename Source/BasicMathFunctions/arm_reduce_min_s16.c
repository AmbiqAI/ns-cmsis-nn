 ...
 * Title:        arm_reduce_min_s16
 * Description:  Reduce min operator for quantized s16 tensors
 *
 * $Date:        23 June 2025
 * $Revision:    V.1.0.0
 *
...
/**
 * @brief Generic fallback reduce min operator for quantized s16 tensors.
 *
 ...
 */
arm_cmsis_nn_status arm_reduce_min_generic_s16(const int16_t *input_data,
...
/**
 * @brief Flatten last dims reduce min operator for quantized s16 tensors.
 *
 * @param[in]  input_data     Pointer to input tensor
 * @param[out] output_data    Pointer to output buffer
 ...
 * @return     ARM_CMSIS_NN_SUCCESS on success
 */
arm_cmsis_nn_status arm_reduce_min_flatten_last_dims_s16(const int16_t *input_data,
                                                         int16_t *output_data,
                                                         int32_t outer_size,
                                                         int32_t inner_size)
{
    ...
    const int16x8_t vmax_s16 = vdupq_n_s16(INT16_MAX);
    for (int i = 0; i < outer_size; ++i)
    {
        ...
        int16_t min_val = INT16_MAX;
        min_val = vminvq_s16(min_val, vmin);
...
/**
 * @brief Spatial reduce min operator for quantized s16 tensors.
 *
 * @param[in]  input_data     Pointer to input tensor
 * @param[in]  input_dims     Input tensor dimensions (4D NHWC)
 * @param[in]  axis_dims      4D binary axis mask (1 = reduce)
 ...
 * @return     ARM_CMSIS_NN_SUCCESS on success
 */
arm_cmsis_nn_status arm_reduce_min_spatial_s16(const int16_t *input_data,
                                               const cmsis_nn_dims *input_dims,
                                               const cmsis_nn_dims *axis_dims,
                                               int16_t *output_data,
                                               const cmsis_nn_dims *output_dims)
{
    ...
    for (int n = 0; n < N; ++n)
    {
        ...
        for (; c <= C - 8; c += 8)
        {
            ...
            // Initialize min vector to INT16_MAX
            int16x8_t minv = vdupq_n_s16(INT16_MAX);

            // Walk over each spatial element, take pairwise min
            for (int i = 0; i < spatial; ++i)
            {
                int16x8_t v = vld1q_s16(in_ptr + i * C + c);
                minv = vminq_s16(minv, v);
            }
            // Store out the 16‐lane min
            vst1q_s16(out_ptr + c, minv);
        }
    ...
    for (int n = 0; n < N; ++n)
    {
        ...
        {
            int32_t s = 0;
            // pack two 16-bit lanes, both init to INT16_MAX
            int32_t min_pair = PKHBT(INT16_MAX, INT16_MAX, 16);
    ...
    return arm_reduce_min_generic_s16(input_data, input_dims, axis_dims, output_data, output_dims);

#endif
}

/*
 * s16 reduce min over the specified axis
 *
                                       ...
                                       const cmsis_nn_dims *output_dims)
{
    ...
    if (suffix_start >= 0)
    {
        ...
        return arm_reduce_min_flatten_last_dims_s16(input_data, output_data, outer_size, inner_size);
    }

    // Check for spatial reduction axis=[H,W]
    if (!axis_dims->n && axis_dims->h && axis_dims->w && !axis_dims->c)
    {
        return arm_reduce_min_spatial_s16(input_data, input_dims, axis_dims, output_data, output_dims);
    }
