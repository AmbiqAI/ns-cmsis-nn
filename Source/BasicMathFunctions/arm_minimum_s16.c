...
arm_min_no_broadcast_s16(const int16_t *input_1, const int16_t *input_2, int16_t *output, int32_t flat_size)
{
    ...
    while (flat_size > 0)
    {
        ...
        vstrhq_p_s16(output, vminq_s16(vec1, vec2), p);
        output += 8;
        flat_size -= 8;
    }
#else
...
static arm_cmsis_nn_status
arm_min_scalar_s16(const int16_t *input_1, const int16_t *input_2, int16_t *output, int32_t flat_size)
{
#if defined(ARM_MATH_MVEI)
    ...
    while (flat_size > 0)
    {
        ...
        vstrhq_p_s16(output, vminq_s16(scalar_vec, vec), p);
        output += 8;
        flat_size -= 8;
    }
#else
                                   ...
                                   const cmsis_nn_dims *output_dims)
{
    ...
    if (arm_check_broadcast_required(input_1_dims, input_2_dims))
    {
        if (flat_size_1 == 1)
        {
            ...
            arm_min_scalar_s16(input_1_data, input_2_data, output_data, flat_size_2);
        }
        else if (flat_size_2 == 1)
        {
            // arm_min_scalar expects the tensor with the scalar value to be provided first
            arm_min_scalar_s16(input_2_data, input_1_data, output_data, flat_size_1);
        }
        else
        {
            ...
            for (int32_t i_out_batch = 0; i_out_batch < output_batch; i_out_batch++)
            {
                ...
                {
                    arm_min_no_broadcast_s16(input_1_ptr, input_2_ptr, output_data, flat_size_1);
                    output_data += flat_size_1;
                }
                else if (flat_size_1 == 1)
                {
                    arm_min_scalar_s16(input_1_ptr, input_2_ptr, output_data, flat_size_2);
                    output_data += flat_size_2;
                }
                else if (flat_size_2 == 1)
                {
                    arm_min_scalar_s16(input_2_ptr, input_1_ptr, output_data, flat_size_1);
                    output_data += flat_size_1;
                }
                else
                {
                    ...
                    for (int32_t i_out_height = 0; i_out_height < output_height; i_out_height++)
                    {
                        ...
                        else
                        {
                            for (int32_t i_out_width = 0; i_out_width < output_width; i_out_width++)
                            {
                                ...
                                {
                                    arm_min_no_broadcast_s16(input_1_ptr, input_2_ptr, output_data, input_1_channels);
                                ...
                                {
                                    // arm_min_scalar expects the tensor with the scalar value to be provided first
                                    arm_min_scalar_s16(input_1_ptr, input_2_ptr, output_data, input_2_channels);
                                ...
                                {
                                    // arm_min_scalar expects the tensor with the scalar value to be provided first
                                    arm_min_scalar_s16(input_2_ptr, input_1_ptr, output_data, input_1_channels);
                                    output_data += input_1_channels;
                                    input_1_ptr += input_1_channels;
                                    input_2_ptr++;
                                }
    ...
    {
        arm_min_no_broadcast_s16(input_1_data, input_2_data, output_data, flat_size_1);
    }
