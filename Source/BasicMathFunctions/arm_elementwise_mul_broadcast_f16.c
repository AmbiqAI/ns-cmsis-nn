 ...
 * Title:        arm_elementwise_mul_broadcast_f16.c
 * Description:  Elementwise multiply for float16 tensors with NHWC broadcasting
 *
...
/* vec * scalar: the arithmetic and clamp of arm_elementwise_mul_f16 with the second operand
 * held in a register, so the result is bit-identical to the flat kernel over a materialised
 * broadcast operand (#415). */
static void arm_elementwise_mul_scalar_f16(const float16_t *vec,
                                           float16_t scalar,
                                           float16_t *output,
                                           ...
                                           int32_t n)
{
    ...
    for (; i <= n - 8; i += 8)
    {
        ...
        vst1q(&output[i], arm_nn_clamp_propagate_nan_mve_f16(vmulq(va, vs), vmin, vmax));
    }
    if (i < n)
    {
        const mve_pred16_t p = vctp16q((uint32_t)(n - i));
        const float16x8_t va = vld1q_z(&vec[i], p);
        vstrhq_p(&output[i], arm_nn_clamp_propagate_nan_mve_f16(vmulq(va, vs), vmin, vmax), p);
    }
    #else
    ...
    #define ARM_MUL_BCAST_F16_FULL(a, b, o, n)                                                                         \
        (void)arm_elementwise_mul_f16((a), (b), (o), out_activation_min, out_activation_max, (n))
    #define ARM_MUL_BCAST_F16_SCALAR_1(s, v, o, n)                                                                     \
        arm_elementwise_mul_scalar_f16((v), *(s), (o), out_activation_min, out_activation_max, (n))
    #define ARM_MUL_BCAST_F16_SCALAR_2(s, v, o, n)                                                                     \
        arm_elementwise_mul_scalar_f16((v), *(s), (o), out_activation_min, out_activation_max, (n))

/*
 * float16 elementwise multiply with NHWC broadcasting.
 *
                                                      ...
                                                      float16_t out_activation_max)
{
    ...
    ARM_NN_BROADCAST_WALK_NHWC(float16_t,
                               ...
                               output_data,
                               output_dims,
                               ARM_MUL_BCAST_F16_FULL,
                               ARM_MUL_BCAST_F16_SCALAR_1,
                               ARM_MUL_BCAST_F16_SCALAR_2);

    return ARM_CMSIS_NN_SUCCESS;
}

    #undef ARM_MUL_BCAST_F16_FULL
    #undef ARM_MUL_BCAST_F16_SCALAR_1
    #undef ARM_MUL_BCAST_F16_SCALAR_2
