...
arm_quantize_f32_s8(const float *input, int8_t *output, int32_t size, int32_t zero_point, float scale)
{
    ...
    if (zero_point < INT8_MIN || zero_point > INT8_MAX)
    {
    ...
    int32x4_t max = vdupq_n_s32(INT8_MAX - zero_point);
    int32x4_t min = vdupq_n_s32(INT8_MIN - zero_point);
    for (int i = 0; i < count; i++)
    {
        ...
        vstrbq_p_s32(output, shifted, pred);
        input += 4;
        output += 4;
    }
#else
    for (int i = 0; i < size; i++)
    {
        ...
        rounded = fminf(fmaxf(rounded, (float)(INT8_MIN - zero_point)), (float)(INT8_MAX - zero_point));
        output[i] = (int8_t)((int32_t)rounded + zero_point);
    }
#endif
