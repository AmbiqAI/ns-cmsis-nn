...
typedef struct
{
    ...
    float16x8_t vmin;
    float16x8_t vmax;
    uint16x8_t lane_x_c;    /* lane * stride_w * input_c */
    uint16x8_t out_offsets; /* lane * output_c */
} arm_conv_small_c_f16;

/* Direct kernel for fewer input channels than one vector: lanes are output x positions, weights are scalars,
 * OC_GROUP output channels accumulate at once in float32 (widened once per load, narrowed once per output). Interior
 * lane groups (every tap in range) load unpredicated -- contiguous for input_c == 1 with stride 1/2, gathered otherwise
 * -- and store unpredicated; edge and tail groups compare in_x against input_w once per tap column. */
__STATIC_INLINE bool arm_conv_nhwc_use_small_c_f16(const cmsis_nn_conv_params_f16 *conv_params,
                                                   const cmsis_nn_dims *input_dims,
                                                   const cmsis_nn_dims *filter_dims,
                                                   const cmsis_nn_dims *output_dims)
{
...
__STATIC_FORCEINLINE void arm_conv_small_c_group_f16(const arm_conv_small_c_f16 *__RESTRICT c,
                                                     const float16_t *__RESTRICT input_b,
                                                     int32_t in_y0,
                                                     int32_t in_x0,
                                                     ...
                                                     mve_pred16_t p_pos)
{
    ...
    float32x4_t vacc0_lo = c->bias ? vdupq_n_f32((float32_t)c->bias[oc0 + oc_idx0]) : vdupq_n_f32(0.0f);
    float32x4_t vacc0_hi = vacc0_lo;
    float32x4_t vacc1_lo = c->bias ? vdupq_n_f32((float32_t)c->bias[oc0 + oc_idx1]) : vdupq_n_f32(0.0f);
    float32x4_t vacc1_hi = vacc1_lo;
    const float16_t *w0 = c->packed ? c->filter + ((size_t)(oc0 / 8) * (size_t)c->patch_len) * 8 + (size_t)(oc0 % 8)
    ...
    const uint16_t row_limit = (uint16_t)(c->input_w * input_c);
    ...
    for (int32_t ky = 0; ky < c->kernel_h; ++ky)
    {
        ...
        const float16_t *row = input_b + (size_t)in_y * row_len;
    ...
    {
        vstrhq_scatter_shifted_offset(out_pos + oc0, out_offsets, arm_nn_clamp_mve_f16(r0, vmin, vmax));
    }
    if (n_oc > 1)
    {
        /* Both half-lanes are written by the two narrowing converts, so any seed will do. */
        ...
        if (MODE == ARM_NN_CONV_SMALL_C_MODE_EDGE)
        {
            vstrhq_scatter_shifted_offset_p(
                out_pos + oc0 + 1, out_offsets, arm_nn_clamp_mve_f16(r1, vmin, vmax), p_pos);
        }
        else
        {
            vstrhq_scatter_shifted_offset(out_pos + oc0 + 1, out_offsets, arm_nn_clamp_mve_f16(r1, vmin, vmax));
        }
    }
}

/* Every output channel of one lane group, OC_GROUP at a time. */
__STATIC_FORCEINLINE void arm_conv_small_c_row_group_f16(const arm_conv_small_c_f16 *__RESTRICT c,
                                                         const float16_t *__RESTRICT input_b,
                                                         int32_t in_y0,
                                                         int32_t in_x0,
                                                         float16_t *__RESTRICT out_pos,
                                                         ...
                                                         mve_pred16_t p_pos)
{
    for (int32_t oc0 = 0; oc0 < c->output_c; oc0 += ARM_NN_CONV_SMALL_C_F16_OC_GROUP)
    {
        const int32_t n_oc = c->output_c - oc0;
        arm_conv_small_c_group_f16(c,
                                   input_b,
                                   in_y0,
                                   in_x0,
                                   out_pos,
                                   oc0,
                                   (n_oc < ARM_NN_CONV_SMALL_C_F16_OC_GROUP) ? n_oc : ARM_NN_CONV_SMALL_C_F16_OC_GROUP,
                                   IC1,
                                   MODE,
                                   p_pos);
    }
...
__STATIC_FORCEINLINE void arm_conv_small_c_lane_group_f16(const arm_conv_small_c_f16 *__RESTRICT c,
                                                          const float16_t *__RESTRICT input_b,
                                                          int32_t in_y0,
                                                          int32_t in_x0,
                                                          float16_t *__RESTRICT out_pos,
                                                          ...
                                                          mve_pred16_t p_pos)
{
    ...
    case ARM_NN_CONV_SMALL_C_MODE_VLD1:
        arm_conv_small_c_row_group_f16(c, input_b, in_y0, in_x0, out_pos, true, ARM_NN_CONV_SMALL_C_MODE_VLD1, p_pos);
        break;
    case ARM_NN_CONV_SMALL_C_MODE_VLD2:
        arm_conv_small_c_row_group_f16(c, input_b, in_y0, in_x0, out_pos, true, ARM_NN_CONV_SMALL_C_MODE_VLD2, p_pos);
        break;
...
arm_nn_conv_small_c_nhwc_f16(const arm_conv_small_c_f16 *__RESTRICT c,
                             int32_t batch,
                             const float16_t *__RESTRICT input_data,
                             float16_t *__RESTRICT output_data)
{
    ...
    const int32_t span = (ARM_NN_CONV_SMALL_C_F16_LANES - 1) * stride_w + (c->kernel_w - 1) * c->dil_w;
    ...
    const int32_t block_span = ARM_NN_CONV_SMALL_C_F16_LANES * stride_w - 1 + (c->kernel_w - 1) * c->dil_w;
    ...
    {
        const float16_t *input_b = input_data + (size_t)b * input_b_len;
        float16_t *output_b = output_data + (size_t)b * output_b_len;
        ...
        for (int32_t out_y = 0; out_y < c->output_h; ++out_y)
        {
            ...
            for (; out_x0 + ARM_NN_CONV_SMALL_C_F16_LANES <= output_w; out_x0 += ARM_NN_CONV_SMALL_C_F16_LANES)
            {
                ...
                arm_conv_small_c_lane_group_f16(
            ...
            if (out_x0 < output_w)
            {
                ...
                arm_conv_small_c_lane_group_f16(c,
                                                input_b,
                                                in_y0,
...
static arm_cmsis_nn_status arm_conv_small_c_dispatch_f16(const cmsis_nn_conv_params_f16 *conv_params,
                                                         const cmsis_nn_dims *input_dims,
                                                         ...
                                                         float16_t *output_data)
{
    ...
    c.w_lane_stride = c.packed ? ARM_NN_CONV_SMALL_C_F16_LANES : 1;
    c.w_oc_step = c.packed ? 1 : c.patch_len;
    c.vmin = vdupq_n_f16(conv_params->activation.min);
    c.vmax = vdupq_n_f16(conv_params->activation.max);
    c.lane_x_c = vmulq(lane, (uint16_t)(c.stride_w * c.input_c));
    c.out_offsets = vmulq(lane, (uint16_t)c.output_c);

    return arm_nn_conv_small_c_nhwc_f16(&c, input_dims->n, input_data, output_data);
}
                ...
                for (int32_t oc = 0; oc < output_c; ++oc)
                {
                    ...
                    for (int32_t ky = 0; ky < kernel_h; ++ky)
                    {
                        ...
                        for (int32_t kx = 0; kx < kernel_w; ++kx)
                        {
                            ...
                            int32_t ic = 0;
                            for (; ic + 8 <= kernel_ch; ic += 8)
                            {
                                vacc = vfmaq(vacc, vld1q(x + ic), vld1q(w_tap + ic));
                            }
                            if (ic < kernel_ch)
                            {
                                const mve_pred16_t p = vctp16q((uint32_t)(kernel_ch - ic));
