                                              ...
                                              int8_t *output)
{
    ...
    int32_t active_ch = ARM_NN_MIN(CH_IN_BLOCK_MVE, remaining_ch);
    ...
    for (int i_ch = 0; i_ch < ch_loop; i_ch++)
    {
        ...
        for (int i_buf = 0; i_buf < buffer_count; i_buf++)
        {
            ...
            if (channel_offset < active_ch)
            {
                ...
                const int32x4_t mult = vldrwq_z_s32(&output_mult[offset], p);
                const int32x4_t shift = vldrwq_z_s32(&output_shift[offset], p);
        ...
        active_ch = ARM_NN_MIN(CH_IN_BLOCK_MVE, remaining_ch);
        remaining_ch -= CH_IN_BLOCK_MVE;
    }
