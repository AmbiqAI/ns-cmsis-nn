...
__STATIC_INLINE bool arm_conv_1x1_nhwc_use_patch_gemm_f32(const cmsis_nn_context *ctx,
                                                          int32_t input_c,
                                                          int32_t output_c,
                                                          int32_t output_positions)
{
    ...
    if (input_c < ARM_NN_CONV_NHWC_PATCH_GEMM_F32_MIN_K || output_c < ARM_NN_CONV_NHWC_PATCH_GEMM_F32_MIN_OC ||
        output_positions < ARM_NN_CONV_NHWC_PATCH_GEMM_F32_MIN_POS)
    {
        return false;
    }

    const size_t row_bytes = (size_t)input_c * sizeof(float32_t);
    return row_bytes > 0U && (size_t)ctx->size >= row_bytes;
}

__STATIC_INLINE arm_cmsis_nn_status arm_convolve_1x1_mat_mul_f32(const float32_t *lhs,
                                                                 const float32_t *rhs,
...
static arm_cmsis_nn_status arm_convolve_1x1_nhwc_patch_gemm_f32(const cmsis_nn_context *ctx,
                                                                const cmsis_nn_conv_params_f32 *conv_params,
    ...
    for (int32_t b = 0; b < batch; ++b)
    {
        ...
        for (int32_t pos = 0; pos < output_positions; pos += tile_rows)
        {
            ...
            for (int32_t r = 0; r < rows; ++r)
            {
                ...
                arm_memcpy_f32(patch_matrix + (size_t)r * input_c, input_b + src, (uint32_t)input_c);
            }

            arm_cmsis_nn_status st = arm_convolve_1x1_mat_mul_f32(patch_matrix,
                                                                  filter_data,
                                                                  bias_data,
                                              ...
                                              float32_t *output_data)
{
    ...
    if (stride_h == 1 && stride_w == 1 && output_h == input_h && output_w == input_w)
    {
        ...
        {
            const float32_t *input_b = input_data + (size_t)b * input_hw * input_c;
            float32_t *output_b = output_data + (size_t)b * output_h * output_w * output_c;
            arm_cmsis_nn_status status = arm_convolve_1x1_mat_mul_f32(
    ...
    if (arm_conv_1x1_nhwc_use_patch_gemm_f32(ctx, input_c, output_c, output_positions))
    {
        return arm_convolve_1x1_nhwc_patch_gemm_f32(
    ...
    for (int32_t b = 0; b < input_dims->n; ++b)
    {
        ...
        for (int32_t out_y = 0; out_y < output_h; ++out_y)
        {
            ...
            for (int32_t out_x = 0; out_x < output_w; ++out_x)
            {
                ...
                float32_t *output_pixel = output_b + ((size_t)out_y * output_w + (size_t)out_x) * output_c;
                arm_cmsis_nn_status status = arm_convolve_1x1_mat_mul_f32(input_b + in_base,
                                                                          filter_data,
                                                                          bias_data,
                                                                          output_pixel,
                                                                          1,
