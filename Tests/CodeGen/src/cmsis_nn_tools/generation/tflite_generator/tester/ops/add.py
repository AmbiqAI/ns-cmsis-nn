...
class OpAdd(OperationBase):
    ...
    def generate_c_files(self, output_dir: Path) -> None:
        ...
        # Activation min/max based on dtype (needed for multiplier reduction check)
        activation_dtype = self.desc.get('activation_dtype', 'S8')
        ...
        # Left shift: typically 0 for add operations
        left_shift = 0
        if activation_dtype == 'S8':
            activation_min = -128
            activation_max = 127
        ...
        h_content = self.render_template("add/add.h.j2", context)
        h_path = includes_api_dir / f"{name}_add.h"
        with open(h_path, 'w') as f:
            f.write(h_content)
        
        c_content = self.render_template("add/add.c.j2", context)
        c_path = output_dir / f"{name}_add.c"
