...
def find_tester_templates_dir(repo_root: Optional[Path] = None) -> Path:
    ...
    Returns:
        Path to tester/templates/ under src/helia_core_tester/generation/tflite_generator/
    """
    if repo_root is None:
        repo_root = find_repo_root()
...
def find_fvp_script_path(repo_root: Optional[Path] = None) -> Path:
    ...
    Returns:
        Path to build_and_run_fvp.py under src/helia_core_tester/fvp/
    """
    if repo_root is None:
        repo_root = find_repo_root()
    
    return repo_root / "src" / "helia_core_tester" / "fvp" / "build_and_run_fvp.py"
...
def find_setup_dependencies_script(repo_root: Optional[Path] = None) -> Optional[Path]:
    ...
    Returns:
        Path to setup_dependencies.py under src/helia_core_tester/scripts/, or None if not found
    """
    if repo_root is None:
        repo_root = find_repo_root()
    
    script = repo_root / "src" / "helia_core_tester" / "scripts" / "setup_dependencies.py"
