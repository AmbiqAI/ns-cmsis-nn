...
from helia_core_tester.generation.tflite_generator.tester.ops.base import OperationBase
...
class OpTanh(OperationBase):
    ...
    def build_keras_model(self) -> tf.keras.Model:
        """Build Keras model for Tanh operation."""
        ...
        # Tanh operation
        output = tf.keras.layers.Activation('tanh')(inputs)
    ...
    def convert_to_tflite(self, model, out_path: str, rep_seed: int) -> None:
        ...
        if activation_dtype == 'S16':
            ...
            raise ValueError(f"Unsupported activation_dtype for Tanh: {activation_dtype} (only S16 supported)")
    ...
    def _select_cmsis_tanh_kernel(self) -> Dict[str, str]:
        """
        Select appropriate CMSIS-NN kernel function for Tanh operation.
    ...
    def generate_c_files(self, output_dir: Path) -> None:
        """
        Generate C and H files from templates for Tanh operation.
        """
        from helia_core_tester.generation.tflite_generator.tester.utils.template_context import TemplateContextBuilder
        from helia_core_tester.generation.tflite_generator.tester.utils.tflite_utils import calculate_multiplier_shift
        ...
        kernel_info = self._select_cmsis_tanh_kernel()
        
        # Load LiteRT model for shape and quantization extraction
        from helia_core_tester.generation.tflite_generator.tester.utils.litert_utils import get_operator_tensors_from_litert
        ...
        def checked_log2(x):
            ...
            if x <= 0:
                return False, 0
            log2_val = math.log(x) / math.log(2.0)
            log2_rounded = int(round(log2_val))
            fracpart = log2_val - log2_rounded
            is_pot = abs(fracpart) < 1e-3
            return is_pot, log2_rounded
        
        is_pot_out, log2_out = checked_log2(output_scale)
        
        # Verify output scale is 2^-15 (or close)
        # For S16 tanh, output_scale should be approximately 1/32768 = 2^-15
        ...
        param_scale_pot = is_pot_in and (calculated_left_shift == 0 or calculated_left_shift == 1)
        ...
        h_content = self.render_template("tanh/tanh.h.j2", context)
        h_path = includes_api_dir / f"{name}_tanh.h"
        with open(h_path, 'w') as f:
            f.write(h_content)
        
        c_content = self.render_template("tanh/tanh.c.j2", context)
