...
from cmsis_nn_tools.core.errors import FVPRunError, PathNotFoundError
from cmsis_nn_tools.core.logging import get_logger
from cmsis_nn_tools.core.discovery import find_fvp_script_path
    ...
    def execute(self) -> StepResult:
        ...
        if self.should_skip():
            return StepResult(
                ...
                message="Test execution skipped (--skip-run)"
            )
        ...
        cmd = [
            sys.executable, "-m", "cmsis_nn_tools.fvp.build_and_run_fvp",
            "--cpu", self.config.cpu,
            "--no-build",
        ]
