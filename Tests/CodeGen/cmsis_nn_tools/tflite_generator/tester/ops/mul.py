...
class OpMul(OperationBase):
    ...
    def generate_c_files(self, output_dir: Path) -> None:
        ...
        input1_qp = input_details[0].get('quantization_parameters', {})
        input2_qp = input_details[1].get('quantization_parameters', {})
        output_qp = output_details[0].get('quantization_parameters', {})
        
        input1_scale = input1_qp.get('scales', [1.0])
        input1_zp = input1_qp.get('zero_points', [0])
        input2_scale = input2_qp.get('scales', [1.0])
        input2_zp = input2_qp.get('zero_points', [0])
        output_scale = output_qp.get('scales', [1.0])
        output_zp = output_qp.get('zero_points', [0])
        
        # Get first element (per-tensor quantization)
        input1_scale = float(input1_scale[0] if isinstance(input1_scale, list) else input1_scale)
        input1_zp = int(input1_zp[0] if isinstance(input1_zp, list) else input1_zp)
        input2_scale = float(input2_scale[0] if isinstance(input2_scale, list) else input2_scale)
        input2_zp = int(input2_zp[0] if isinstance(input2_zp, list) else input2_zp)
        ...
        # For elementwise mul, effective scale is: (input1_scale * input2_scale) / output_scale
        ...
        h_content = self.render_template("mul/mul.h.j2", context)
        h_path = includes_api_dir / f"{name}_mul.h"
        with open(h_path, 'w') as f:
            f.write(h_content)
        
        c_content = self.render_template("mul/mul.c.j2", context)
        c_path = output_dir / f"{name}_mul.c"
