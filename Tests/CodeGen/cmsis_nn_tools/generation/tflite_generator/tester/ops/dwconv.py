...
class OpDepthwiseConv2D(OperationBase):
    ...
    def generate_c_files(self, output_dir: Path) -> None:
        ...
        if biases is None or (hasattr(biases, 'shape') and len(biases.shape) == 1 and biases.shape[0] != expected_output_channels):
            from cmsis_nn_tools.generation.tflite_generator.tester.utils.litert_utils import get_tensor_data_from_litert, get_tensor_shape_from_litert
        ...
        if per_channel and isinstance(weight_scale, np.ndarray):
            ...
            from cmsis_nn_tools.generation.tflite_generator.tester.utils.tflite_utils import calculate_per_channel_multiplier_shift
