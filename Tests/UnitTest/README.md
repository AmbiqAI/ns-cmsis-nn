...
python3 run_integer_unit_tests.py \
  ...
  --run-fvp \
  --toolchains GCC \
  --cmsis-pack-root /path/to/ns-cmsis-nn/Tests/UnitTest/downloads \
...
RSQRT s16 is available through the top-level script and is generated by the refactored helper in `RefactoredTestGen`:
...
You can also invoke the refactored RSQRT generator directly:

```
./RefactoredTestGen/generate_rsqrt_s16_test_data.py
```
...
| maximum         |      |  x  |
| transpose       |      |  x  |
| rsqrt           |  x   |  x  | Refactored generator uses TFLite interpreter outputs and independent LUT sampling
