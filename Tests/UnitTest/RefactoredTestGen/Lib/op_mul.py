...
class Op_mul(Lib.op_utils.Op_type):
    ...
    def generate_keras_model(shapes, params):
        ...
        # 2) Apply the Mul layer
        prod = tf.keras.layers.Multiply(name="lhs_x_rhs")([input_lhs, input_rhs])

        # 3) Wrap into a Model
        model = tf.keras.Model(
            inputs=[input_lhs, input_rhs],
            outputs=prod,
            name="simple_mul_model"
        )
    ...
    def generate_data_tflite(tflite_fname, params):
        ...
        # extract for each tensor
        in_det  = interpreter.get_input_details()
        out_det = interpreter.get_output_details()

        # assume a 2-input add
        ...
        real_multiplier = lhs_scale * rhs_scale / out_scale
        out_mult, out_shift = Lib.op_utils.compute_multiplier_shift(real_multiplier)
