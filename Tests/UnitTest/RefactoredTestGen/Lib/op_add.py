...
class Op_add(Lib.op_utils.Op_type):
    ...
    def generate_keras_model(shapes, params):
        ...
        # 2) Apply the Add layer
        added = tf.keras.layers.Add(name="lhs_plus_rhs")([input_lhs, input_rhs])

        # 3) Wrap into a Model
        model = tf.keras.Model(
            inputs=[input_lhs, input_rhs],
            outputs=added,
            name="simple_add_model"
        )
    ...
    def generate_data_tflite(tflite_fname, params):
        ...
        out_multiplier = twice_max_input_scale / ((1 << left_shift) * out_scale)
