...
static float16_t resize_nn_f16_input[RESIZE_NN_FLT_MAX_INPUT];
static float16_t resize_nn_f16_output[RESIZE_NN_FLT_MAX_OUTPUT + RESIZE_NN_GUARD];
static float16_t resize_nn_f16_expected[RESIZE_NN_FLT_MAX_OUTPUT];
static int32_t resize_nn_f16_scratch[RESIZE_NN_FLT_MAX_SCRATCH_ELEMENTS + RESIZE_NN_GUARD];

static void resize_nn_f16_run_case(const resize_nn_flt_case *c, uint32_t seed)
{
    ...
    const int32_t scratch_bytes = arm_resize_nearest_neighbor_f16_get_buffer_size(&output_dims);
    TEST_ASSERT_EQUAL_INT32_MESSAGE((c->out_h + c->out_w) * (int32_t)sizeof(int32_t), scratch_bytes, c->name);
    const cmsis_nn_context ctx = {.buf = resize_nn_f16_scratch, .size = scratch_bytes};

    resize_nn_fill_bits(resize_nn_f16_input, in_elements, sizeof(float16_t), seed);
    resize_nn_gather(resize_nn_f16_expected, resize_nn_f16_input, sizeof(float16_t), c);
    for (size_t i = 0; i < RESIZE_NN_FLT_MAX_OUTPUT + RESIZE_NN_GUARD; ++i)
    {
        const uint16_t canary = (uint16_t)RESIZE_NN_CANARY_WORD;
        memcpy(&resize_nn_f16_output[i], &canary, sizeof(canary));
    }
    ...
    TEST_ASSERT_EQUAL_MESSAGE(ARM_CMSIS_NN_SUCCESS,
                              arm_resize_nearest_neighbor_f16(&ctx,
                                                              &params,
                                                              &input_dims,
                                                              resize_nn_f16_input,
                                                              &output_size_dims,
    ...
    TEST_ASSERT_EQUAL_MEMORY_MESSAGE(
        resize_nn_f16_expected, resize_nn_f16_output, out_elements * sizeof(float16_t), c->name);
    for (size_t i = out_elements; i < RESIZE_NN_FLT_MAX_OUTPUT + RESIZE_NN_GUARD; ++i)
    {
        uint16_t bits;
        memcpy(&bits, &resize_nn_f16_output[i], sizeof(bits));
        TEST_ASSERT_EQUAL_HEX16_MESSAGE((uint16_t)RESIZE_NN_CANARY_WORD, bits, c->name);
    }
    for (size_t i = (size_t)(c->out_h + c->out_w); i < RESIZE_NN_FLT_MAX_SCRATCH_ELEMENTS + RESIZE_NN_GUARD; ++i)
    {
        TEST_ASSERT_EQUAL_HEX32_MESSAGE(RESIZE_NN_CANARY_WORD, (uint32_t)resize_nn_f16_scratch[i], c->name);
    }
}

void resize_nearest_neighbor_f16_tflite_cases(void)
{
    for (int32_t i = 0; i < RESIZE_NN_FLT_CASE_COUNT; ++i)
    {
        resize_nn_f16_run_case(&resize_nn_flt_cases[i], 0x9E3779B9U + (uint32_t)i);
    }
}
...
void resize_nearest_neighbor_f16_special_values(void)
{
    ...
    const int32_t output_size[2] = {c->out_h, c->out_w};
    const cmsis_nn_dims output_dims = {c->n, c->out_h, c->out_w, c->c};
    const cmsis_nn_context ctx = {.buf = resize_nn_f16_scratch,
                                  .size = arm_resize_nearest_neighbor_f16_get_buffer_size(&output_dims)};
    const size_t out_elements = (size_t)c->n * c->out_h * c->out_w * c->c;

    memset(resize_nn_f16_input, 0, sizeof(resize_nn_f16_input));
    for (int32_t pixel = 0; pixel < c->n * c->h * c->w; ++pixel)
    {
        for (int32_t ch = 0; ch < c->c; ++ch)
        {
            memcpy(&resize_nn_f16_input[pixel * c->c + ch],
                   &resize_nn_special_bits_f16[(ch + pixel) % 8],
                   sizeof(float16_t));
        }
    }
    resize_nn_gather(resize_nn_f16_expected, resize_nn_f16_input, sizeof(float16_t), c);
    memset(resize_nn_f16_output, 0, sizeof(resize_nn_f16_output));
    ...
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_SUCCESS,
                      arm_resize_nearest_neighbor_f16(&ctx,
                                                      ...
                                                      &output_dims,
                                                      resize_nn_f16_output));
    TEST_ASSERT_EQUAL_MEMORY(resize_nn_f16_expected, resize_nn_f16_output, out_elements * sizeof(float16_t));
}

void resize_nearest_neighbor_f16_rejects_bad_args(void)
{
    ...
    cmsis_nn_context ctx = {.buf = resize_nn_f16_scratch, .size = 8 * (int32_t)sizeof(int32_t)};
    const uint16_t canary = (uint16_t)RESIZE_NN_CANARY_WORD;
...
#define RESIZE_NN_CALL(CTX, PARAMS, IN_DIMS, IN, SIZE_DIMS, SIZE, OUT_DIMS, OUT)                                       \
    arm_resize_nearest_neighbor_f16((CTX), (PARAMS), (IN_DIMS), (IN), (SIZE_DIMS), (SIZE), (OUT_DIMS), (OUT))
#define RESIZE_NN_EXPECT_ARG_ERROR(...) TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR, RESIZE_NN_CALL(__VA_ARGS__))
    ...
    RESIZE_NN_EXPECT_ARG_ERROR(
        &ctx, &params, NULL, resize_nn_f16_input, &output_size_dims, output_size, &output_dims, resize_nn_f16_output);
    RESIZE_NN_EXPECT_ARG_ERROR(
        &ctx, &params, &input_dims, NULL, &output_size_dims, output_size, &output_dims, resize_nn_f16_output);
    RESIZE_NN_EXPECT_ARG_ERROR(
        &ctx, &params, &input_dims, resize_nn_f16_input, NULL, output_size, &output_dims, resize_nn_f16_output);
    RESIZE_NN_EXPECT_ARG_ERROR(
        &ctx, &params, &input_dims, resize_nn_f16_input, &output_size_dims, NULL, &output_dims, resize_nn_f16_output);
    RESIZE_NN_EXPECT_ARG_ERROR(
        &ctx, &params, &input_dims, resize_nn_f16_input, &output_size_dims, output_size, NULL, resize_nn_f16_output);
    RESIZE_NN_EXPECT_ARG_ERROR(
        &ctx, &params, &input_dims, resize_nn_f16_input, &output_size_dims, output_size, &output_dims, NULL);
    ...
    RESIZE_NN_EXPECT_ARG_ERROR(&bad_ctx,
                               ...
                               &output_dims,
                               resize_nn_f16_output);
    bad_ctx.buf = resize_nn_f16_scratch;
    ...
    RESIZE_NN_EXPECT_ARG_ERROR(&bad_ctx,
                               ...
                               &output_dims,
                               resize_nn_f16_output);
    bad_ctx.buf = (uint8_t *)resize_nn_f16_scratch + 2;
    ...
    RESIZE_NN_EXPECT_ARG_ERROR(
        &ctx, &params, &input_dims, resize_nn_f16_input, &three_dims, output_size, &output_dims, resize_nn_f16_output);
    const int32_t zero_h[2] = {0, 6};
    const cmsis_nn_dims zero_h_dims = {1, 0, 6, 2};
    RESIZE_NN_EXPECT_ARG_ERROR(
        &ctx, &params, &input_dims, resize_nn_f16_input, &output_size_dims, zero_h, &zero_h_dims, resize_nn_f16_output);
    const int32_t neg_w[2] = {2, -6};
    const cmsis_nn_dims neg_w_dims = {1, 2, -6, 2};
    RESIZE_NN_EXPECT_ARG_ERROR(
        &ctx, &params, &input_dims, resize_nn_f16_input, &output_size_dims, neg_w, &neg_w_dims, resize_nn_f16_output);
...
void resize_nearest_neighbor_f16_buffer_size(void)
{
    const cmsis_nn_dims dims = {1, 7, 9, 3};
    TEST_ASSERT_EQUAL_INT32(16 * (int32_t)sizeof(int32_t), arm_resize_nearest_neighbor_f16_get_buffer_size(&dims));
    const cmsis_nn_dims one = {4, 1, 1, 64};
    TEST_ASSERT_EQUAL_INT32(2 * (int32_t)sizeof(int32_t), arm_resize_nearest_neighbor_f16_get_buffer_size(&one));
    TEST_ASSERT_EQUAL_INT32(-1, arm_resize_nearest_neighbor_f16_get_buffer_size(NULL));
    const cmsis_nn_dims zero_h = {1, 0, 9, 3};
    TEST_ASSERT_EQUAL_INT32(-1, arm_resize_nearest_neighbor_f16_get_buffer_size(&zero_h));
    const cmsis_nn_dims neg_w = {1, 7, -1, 3};
    TEST_ASSERT_EQUAL_INT32(-1, arm_resize_nearest_neighbor_f16_get_buffer_size(&neg_w));
    const cmsis_nn_dims huge = {1, INT32_MAX, 1, 1};
    TEST_ASSERT_EQUAL_INT32(-1, arm_resize_nearest_neighbor_f16_get_buffer_size(&huge));
    const cmsis_nn_dims huge_product = {1, INT32_MAX / 2, INT32_MAX / 2, 1};
    TEST_ASSERT_EQUAL_INT32(-1, arm_resize_nearest_neighbor_f16_get_buffer_size(&huge_product));
}
