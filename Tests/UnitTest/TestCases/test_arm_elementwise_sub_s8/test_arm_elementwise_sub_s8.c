...
#include "../TestData/sub/test_data.h"
#include "../Utils/validate.h"

void sub_arm_elementwise_sub_s8(void)
{
    const arm_cmsis_nn_status expected = ARM_CMSIS_NN_SUCCESS;
    int8_t output[SUB_DST_SIZE] = {0};

    const int8_t *input_data1 = sub_input1_data;
    const int8_t *input_data2 = sub_input2_data;

    const int32_t input_1_mult = SUB_INPUT1_MULT;
    const int32_t input_1_shift = SUB_INPUT1_SHIFT;
    const int32_t input_1_offset = SUB_INPUT1_OFFSET;
    const int32_t input_2_mult = SUB_INPUT2_MULT;
    const int32_t input_2_shift = SUB_INPUT2_SHIFT;
    const int32_t input_2_offset = SUB_INPUT2_OFFSET;

    const int32_t left_shift = SUB_LEFT_SHIFT;

    const int32_t out_offset = SUB_OUTPUT_OFFSET;
    const int32_t out_mult = SUB_OUTPUT_MULT;
    const int32_t out_shift = SUB_OUTPUT_SHIFT;

    const int32_t out_activation_min = SUB_ACTIVATION_MIN;
    const int32_t out_activation_max = SUB_ACTIVATION_MAX;

    arm_cmsis_nn_status result = arm_elementwise_sub_s8(input_data1,
                                                        input_data2,
                                                        input_1_offset,
                                                        input_1_mult,
                                                        input_1_shift,
    ...
    TEST_ASSERT_TRUE(validate(output, sub_output_ref, SUB_DST_SIZE));
}

/* Block-size invariance for the s8 sub primitives.
 *
