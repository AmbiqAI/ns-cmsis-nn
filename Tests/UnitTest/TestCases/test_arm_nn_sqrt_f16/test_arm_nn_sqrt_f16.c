...
#define SQRT_F16_EXHAUSTIVE_BLOCK 1024
...
static uint16_t sqrt_f16_reference_bits(uint16_t in_bits)
{
...
{
    float16_t output[SQRT_F16_DST_SIZE] = {0};

    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_SUCCESS, arm_nn_sqrt_f16(sqrt_f16_input, output, SQRT_F16_DST_SIZE));
    for (int32_t i = 0; i < SQRT_F16_DST_SIZE; ++i)
    {
        TEST_ASSERT_EQUAL_HEX16(sqrt_f16_bits(sqrt_f16_output_ref[i]), sqrt_f16_bits(output[i]));
    }
}

void sqrt_f16_in_place_arm_nn_sqrt_f16(void)
{
    float16_t values[SQRT_F16_DST_SIZE];
    memcpy(values, sqrt_f16_input, sizeof(values));

    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_SUCCESS, arm_nn_sqrt_f16(values, values, SQRT_F16_DST_SIZE));
    for (int32_t i = 0; i < SQRT_F16_DST_SIZE; ++i)
    {
        TEST_ASSERT_EQUAL_HEX16(sqrt_f16_bits(sqrt_f16_output_ref[i]), sqrt_f16_bits(values[i]));
    }
}
...
void sqrt_f16_special_values_arm_nn_sqrt_f16(void)
{
    ...
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_SUCCESS, arm_nn_sqrt_f16(input, output, count));
...
void sqrt_f16_exhaustive_arm_nn_sqrt_f16(void)
{
    ...
    for (uint32_t base = 0; base < 0x10000u; base += SQRT_F16_EXHAUSTIVE_BLOCK)
    {
        for (int32_t i = 0; i < SQRT_F16_EXHAUSTIVE_BLOCK; ++i)
        {
            const uint16_t bits = (uint16_t)(base + (uint32_t)i);
            memcpy(&input[i], &bits, sizeof(bits));
            output[i] = (float16_t)0.0f;
        }
        TEST_ASSERT_EQUAL(ARM_CMSIS_NN_SUCCESS, arm_nn_sqrt_f16(input, output, SQRT_F16_EXHAUSTIVE_BLOCK));
        for (int32_t i = 0; i < SQRT_F16_EXHAUSTIVE_BLOCK; ++i)
        {
            const uint16_t bits = (uint16_t)(base + (uint32_t)i);
            const uint16_t expected = sqrt_f16_reference_bits(bits);
            const uint16_t actual = sqrt_f16_bits(output[i]);
            if (expected != actual)
            {
...
void sqrt_f16_arg_error_arm_nn_sqrt_f16(void)
{
    ...
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR, arm_nn_sqrt_f16(NULL, output, SQRT_F16_DST_SIZE));
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR, arm_nn_sqrt_f16(sqrt_f16_input, NULL, SQRT_F16_DST_SIZE));
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR, arm_nn_sqrt_f16(sqrt_f16_input, output, 0));
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR, arm_nn_sqrt_f16(sqrt_f16_input, output, -1));
}
