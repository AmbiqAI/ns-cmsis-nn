...
#define LSTM_STEP_F16_HIDDEN 10

static void lstm_step_f16_fill_gate(cmsis_nn_lstm_gate_f16 *gate,
...
void lstm_step_uniform_units_f16(void)
{
    static const float16_t zero_weights[LSTM_STEP_F16_HIDDEN * LSTM_STEP_F16_HIDDEN] = {0};
    static const float16_t bias_open[LSTM_STEP_F16_HIDDEN] = {(float16_t)8.0f,
                                                              (float16_t)8.0f,
                                                              (float16_t)8.0f,
                                                              (float16_t)8.0f,
                                                              (float16_t)8.0f,
                                                              ...
                                                              (float16_t)8.0f,
                                                              (float16_t)8.0f};
    static const float16_t bias_closed[LSTM_STEP_F16_HIDDEN] = {(float16_t)-8.0f,
                                                                (float16_t)-8.0f,
                                                                (float16_t)-8.0f,
                                                                (float16_t)-8.0f,
                                                                (float16_t)-8.0f,
                                                                ...
                                                                (float16_t)-8.0f,
                                                                (float16_t)-8.0f};
    const float16_t input[1] = {(float16_t)0.0f};
    // Both state arrays carry a guard region past hidden_size so an over-wide predicated store is caught.
    float16_t cell_state[LSTM_STEP_F16_HIDDEN + 8];
    float16_t hidden_out[LSTM_STEP_F16_HIDDEN + 8];
    ...
    params.cell_clip = (float16_t)0.0f;
    ...
    lstm_step_f16_fill_gate(&params.output_gate, zero_weights, bias_open, ARM_NN_FLT_ACT_SIGMOID);

    for (int32_t h = 0; h < LSTM_STEP_F16_HIDDEN; h++)
    {
        cell_state[h] = (float16_t)1.5f;
        hidden_out[h] = (float16_t)0.0f;
    }
    for (int32_t h = LSTM_STEP_F16_HIDDEN; h < LSTM_STEP_F16_HIDDEN + 8; h++)
    {
        cell_state[h] = (float16_t)-7.0f;
        hidden_out[h] = (float16_t)-7.0f;
    }
    cmsis_nn_lstm_context_f16 buffers = {.cell_state = cell_state};

    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_SUCCESS, arm_nn_lstm_step_f16(input, NULL, hidden_out, &params, &buffers, 1));
    ...
    uint16_t unit_0_bits;
    memcpy(&unit_0_bits, &hidden_out[0], sizeof(unit_0_bits));
    for (int32_t h = 1; h < LSTM_STEP_F16_HIDDEN; h++)
    {
        uint16_t unit_bits;
...
void lstm_step_two_batches_clipped_f16(void)
{
    ...
    static const float16_t bias_closed[LSTM_STEP_F16_HIDDEN] = {(float16_t)-8.0f,
                                                                ...
                                                                (float16_t)-8.0f,
                                                                (float16_t)-8.0f};
    const float16_t input[4] = {(float16_t)0.0f, (float16_t)0.0f, (float16_t)0.0f, (float16_t)0.0f};
    float16_t cell_state[2 * LSTM_STEP_F16_HIDDEN];
    float16_t hidden_out[4 * LSTM_STEP_F16_HIDDEN];
    ...
    for (int32_t i = 0; i < 2 * LSTM_STEP_F16_HIDDEN; i++)
    {
        cell_state[i] = (float16_t)1.5f;
    }
    for (int32_t i = 0; i < 4 * LSTM_STEP_F16_HIDDEN; i++)
    {
        hidden_out[i] = (float16_t)-7.0f;
    }
    cmsis_nn_lstm_context_f16 buffers = {.cell_state = cell_state};

    // batch_offset 2: batch b's output row lives at hidden_out + b * 2 * hidden_size (batch-major, 2 steps)
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_SUCCESS, arm_nn_lstm_step_f16(input, NULL, hidden_out, &params, &buffers, 2));

    for (int32_t b = 0; b < 2; b++)
    {
        const float16_t *row = hidden_out + b * 2 * LSTM_STEP_F16_HIDDEN;
        uint16_t unit_0_bits;
...
void lstm_step_arg_error_f16(void)
{
    float16_t data[LSTM_STEP_F16_HIDDEN] = {0};
    cmsis_nn_lstm_params_f16 params;
    memset(&params, 0, sizeof(params));
    params.batch_size = 1;
    params.input_size = 1;
    params.hidden_size = LSTM_STEP_F16_HIDDEN;
    cmsis_nn_lstm_context_f16 buffers = {.cell_state = data};
    cmsis_nn_lstm_context_f16 no_cell = {.cell_state = NULL};

    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR, arm_nn_lstm_step_f16(NULL, NULL, data, &params, &buffers, 1));
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR, arm_nn_lstm_step_f16(data, NULL, data, &params, &no_cell, 1));
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR, arm_nn_lstm_step_f16(data, NULL, data, &params, &buffers, 0));
}
