// Generated by generate_test_data.py using tensorflow version 2.10.0 (Keras version 2.10.0).
// Interpreter from tensorflow version 2.10.0 and revision upstream/v2.10.0-0-g359c3cdfc5f.
#pragma once
#define DEPTHWISE_KERNEL_3X3_IN_CH 5
#define DEPTHWISE_KERNEL_3X3_PERF_IN_CH 8
#define DEPTHWISE_KERNEL_3X3_OUT_CH 5
#define DEPTHWISE_KERNEL_3X3_PERF_OUT_CH DEPTHWISE_KERNEL_3X3_PERF_IN_CH
#define DEPTHWISE_KERNEL_3X3_INPUT_W 4
#define DEPTHWISE_KERNEL_3X3_PERF_INPUT_W 49
#define DEPTHWISE_KERNEL_3X3_INPUT_H 5
#define DEPTHWISE_KERNEL_3X3_PERF_INPUT_H 10
#define DEPTHWISE_KERNEL_3X3_DST_SIZE 30
#define DEPTHWISE_KERNEL_3X3_INPUT_BATCHES 1
#define DEPTHWISE_KERNEL_3x3_PERF_INPUT_BATCHES 1
#define DEPTHWISE_KERNEL_3X3_PERF_DST_SIZE (DEPTHWISE_KERNEL_3x3_PERF_INPUT_BATCHES * DEPTHWISE_KERNEL_3X3_PERF_OUTPUT_W * DEPTHWISE_KERNEL_3X3_PERF_OUTPUT_H * DEPTHWISE_KERNEL_3X3_PERF_OUT_CH)
#define DEPTHWISE_KERNEL_3X3_INPUT_SIZE 100
#define DEPTHWISE_KERNEL_3X3_PERF_INPUT_SIZE (DEPTHWISE_KERNEL_3x3_PERF_INPUT_BATCHES * DEPTHWISE_KERNEL_3X3_PERF_INPUT_W * DEPTHWISE_KERNEL_3X3_PERF_INPUT_H * DEPTHWISE_KERNEL_3X3_PERF_IN_CH)
#define DEPTHWISE_KERNEL_3X3_OUT_ACTIVATION_MIN -104
#define DEPTHWISE_KERNEL_3X3_OUT_ACTIVATION_MAX 127
#define DEPTHWISE_KERNEL_3X3_FILTER_X 3
#define DEPTHWISE_KERNEL_3X3_FILTER_Y 3
#define DEPTHWISE_KERNEL_3X3_STRIDE_X 2
#define DEPTHWISE_KERNEL_3X3_STRIDE_Y 2
#define DEPTHWISE_KERNEL_3X3_PAD_X 0
#define DEPTHWISE_KERNEL_3X3_PAD_Y 1
#define DEPTHWISE_KERNEL_3X3_OUTPUT_W 2
#define DEPTHWISE_KERNEL_3X3_PERF_OUTPUT_W (DEPTHWISE_KERNEL_3X3_PERF_INPUT_W - 2)
#define DEPTHWISE_KERNEL_3X3_OUTPUT_H 3
#define DEPTHWISE_KERNEL_3X3_PERF_OUTPUT_H (DEPTHWISE_KERNEL_3X3_PERF_INPUT_H - 2)
#define DEPTHWISE_KERNEL_3X3_CH_MULT 1
#define DEPTHWISE_KERNEL_3X3_INPUT_OFFSET 128
#define DEPTHWISE_KERNEL_3X3_OUTPUT_OFFSET -26
#define DEPTHWISE_KERNEL_3X3_DILATION_X 1
#define DEPTHWISE_KERNEL_3X3_DILATION_Y 1
