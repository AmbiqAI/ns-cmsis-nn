...
// Float16 subnormal inputs: the gate is 0.5, the float32 result is exact,
// and the single narrowing rounds to nearest-even (the smallest subnormal
// halves to a signed zero on the tie).
...
// The compute-in-float32, round-once contract: at the witness input a native
// float16 evaluation (gate rounded to f16, product multiplied in f16) lands
// one ulp away from the correctly rounded value the kernel must produce.
void hard_swish_f16_round_once_arm_hard_swish_f16(void)
{
    ...
    // Belt and braces: the double-rounded value really is different.
