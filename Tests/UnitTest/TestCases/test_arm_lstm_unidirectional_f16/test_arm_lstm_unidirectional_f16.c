...
// arm_lstm_unidirectional_f16 must reject a non-positive batch, input or hidden size and a negative time
// step count before it zeroes the cell state, the same contract arm_gru_unidirectional_f16 has. With
...
void lstm_unidirectional_arg_error_f16(void)
{
    ...
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_SUCCESS, arm_lstm_unidirectional_f16(input, output, &params, &buffers));

    params.batch_size = -1;
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR, arm_lstm_unidirectional_f16(input, output, &params, &buffers));
    params.batch_size = 1;
