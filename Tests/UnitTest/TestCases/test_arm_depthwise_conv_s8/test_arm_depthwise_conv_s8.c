...
void weight_presum(void)
{
    ...
    const int32_t buf_sz =
        arm_depthwise_conv_wrapper_s8_get_buffer_size(&dw_conv_params, &in_dims, &filt_dims, &out_dims);
    ...
    arm_cmsis_nn_status result = arm_depthwise_convolve_weight_sum(
        sum_buf, scratch_buf, kernel, &dw_conv_params, &in_dims, &filt_dims, &out_dims, lhs_offset, bias);
    free(scratch_buf);
...
void single_in_many_out_ch(void)
{
    ...
    const int32_t *bias_data =
        get_bias_address(depthwise_single_in_many_out_ch_biases, DEPTHWISE_SINGLE_IN_MANY_OUT_CH_OUT_CH);
    ...
    int8_t *kernel_data = malloc(DEPTHWISE_SINGLE_IN_MANY_OUT_CH_OUT_CH * DEPTHWISE_SINGLE_IN_MANY_OUT_CH_FILTER_X *
                                 DEPTHWISE_SINGLE_IN_MANY_OUT_CH_FILTER_Y);
    ...
    arm_cmsis_nn_status status =
        arm_transpose_s8(conv_kernel_data, kernel_data, &filter_input_dims, &filter_dims, &transpose_params);
