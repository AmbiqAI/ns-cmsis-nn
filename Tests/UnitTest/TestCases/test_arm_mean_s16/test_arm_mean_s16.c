...
#include "../TestData/mean_axis_h_mixed_s16/test_data.h"
...
/* Inner size 12: one full 8-wide block followed by a 4-element predicated tail. */
void mean_axis_h_mixed_arm_mean_s16(void)
{
    const arm_cmsis_nn_status expected = ARM_CMSIS_NN_SUCCESS;
    int16_t output_data[MEAN_AXIS_H_MIXED_S16_OUTPUT_SIZE] = {0};
    int16_t *output_ptr = output_data;

    const cmsis_nn_dims input_dims = MEAN_AXIS_H_MIXED_S16_IN_DIM;
    const cmsis_nn_dims axis_dims = MEAN_AXIS_H_MIXED_S16_AXIS_DIM;
    const cmsis_nn_dims output_dims = MEAN_AXIS_H_MIXED_S16_OUT_DIM;

    const int16_t *input_data = mean_axis_h_mixed_s16_input_tensor;
    const int16_t *const output_ref = mean_axis_h_mixed_s16_output;
    const int32_t output_ref_size = MEAN_AXIS_H_MIXED_S16_OUTPUT_SIZE;

    arm_cmsis_nn_status result = arm_mean_s16(
        input_data,
        &input_dims,
        MEAN_AXIS_H_MIXED_S16_INPUT_OFFSET,
        &axis_dims,
        output_ptr,
        &output_dims,
        MEAN_AXIS_H_MIXED_S16_OUTPUT_OFFSET,
        MEAN_AXIS_H_MIXED_S16_OUTPUT_MULTIPLIER,
        MEAN_AXIS_H_MIXED_S16_OUTPUT_SHIFT
    );
