...
// The temp1/temp2 scratch is sized per test from the published queries: the buffers hold int16_t gate
// vectors, so the previous shared int8_t arrays sized in s8 elements survived only on time-step slack.
...
void lstm_1(void)
{
    ...
    const int32_t temp1_size = arm_lstm_unidirectional_s8_temp1_get_buffer_size(&params);
    const int32_t temp2_size = arm_lstm_unidirectional_s8_temp2_get_buffer_size(&params);
    /* The published queries must agree with the size this test derives by hand from the kernel's writes: the
       ...
       time-major layer). */
    TEST_ASSERT_EQUAL((LSTM_1_TIME_MAJOR ? LSTM_1_BATCH_SIZE : 1) * LSTM_1_HIDDEN_SIZE * (int32_t)sizeof(int16_t),
                      temp1_size);
    TEST_ASSERT_EQUAL(temp1_size, temp2_size);

    int16_t cell_state[LSTM_1_BATCH_SIZE * LSTM_1_HIDDEN_SIZE];
...
void lstm_2(void)
{
    ...
    /* The published queries must agree with the size this test derives by hand from the kernel's writes: the
       temp buffers hold int16_t gate vectors of the step kernel's batch extent (the layer batch only for a
       time-major layer). */
    TEST_ASSERT_EQUAL((LSTM_2_TIME_MAJOR ? LSTM_2_BATCH_SIZE : 1) * LSTM_2_HIDDEN_SIZE * (int32_t)sizeof(int16_t),
                      temp1_size);
    TEST_ASSERT_EQUAL(temp1_size, temp2_size);

    int16_t cell_state[LSTM_2_BATCH_SIZE * LSTM_2_HIDDEN_SIZE];
...
void lstm_one_time_step(void)
{
    ...
    /* The published queries must agree with the size this test derives by hand from the kernel's writes: the
       temp buffers hold int16_t gate vectors of the step kernel's batch extent (the layer batch only for a
       time-major layer). */
    TEST_ASSERT_EQUAL((LSTM_ONE_TIME_STEP_TIME_MAJOR ? LSTM_ONE_TIME_STEP_BATCH_SIZE : 1) * LSTM_ONE_TIME_STEP_HIDDEN_SIZE * (int32_t)sizeof(int16_t),
                      temp1_size);
    TEST_ASSERT_EQUAL(temp1_size, temp2_size);

    int16_t cell_state[LSTM_ONE_TIME_STEP_BATCH_SIZE * LSTM_ONE_TIME_STEP_HIDDEN_SIZE];
...
void lstm_stateful_batch_major_multibatch(void)
{
    ...
    /* Batch-major layer: the step kernel runs one batch at a time, so the queries report a single batch of
       int16_t gate vectors even though the layer batch is LSTM_STATEFUL_BATCH_MAJOR_MULTIBATCH_BATCH_SIZE. */
    TEST_ASSERT_EQUAL((LSTM_STATEFUL_BATCH_MAJOR_MULTIBATCH_TIME_MAJOR ? LSTM_STATEFUL_BATCH_MAJOR_MULTIBATCH_BATCH_SIZE : 1) * LSTM_STATEFUL_BATCH_MAJOR_MULTIBATCH_HIDDEN_SIZE * (int32_t)sizeof(int16_t),
                      temp1_size);
    TEST_ASSERT_EQUAL(temp1_size, temp2_size);

    /* Streaming call: the caller owns the cell state, seeded to zero for a fresh sequence. */
    int16_t cell_state[LSTM_STATEFUL_BATCH_MAJOR_MULTIBATCH_BATCH_SIZE * LSTM_STATEFUL_BATCH_MAJOR_MULTIBATCH_HIDDEN_SIZE];
    ...
    arm_cmsis_nn_status result =
        arm_lstm_unidirectional_s8(lstm_stateful_batch_major_multibatch_input_tensor, output, &params, &buffers);
