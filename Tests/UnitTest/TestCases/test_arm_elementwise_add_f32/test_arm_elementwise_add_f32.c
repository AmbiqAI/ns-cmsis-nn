 ...
 */
static bool add_f32_bits_are_nan(float32_t x)
{
    uint32_t bits;
 ...
 */
static float32_t add_f32_from_bits(volatile const uint32_t *bits)
{
...
void add_f32_arm_elementwise_add_f32(void)
{
    // Exactly representable values, so the reference is bit-exact; the last lanes
    // exercise both clamp bounds.
    const float32_t in1[9] = {0.5f, -2.0f, 3.25f, 1.75f, -0.125f, 8.0f, -8.0f, 2.5f, 0.0f};
    const float32_t in2[9] = {0.25f, 1.0f, -1.5f, 0.25f, 0.375f, 1.0f, -1.0f, -2.5f, 0.0f};
    const float32_t ref[9] = {0.75f, -1.0f, 1.75f, 2.0f, 0.25f, 6.0f, -6.0f, 0.0f, 0.0f};
    float32_t output[9] = {0};

    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_SUCCESS, arm_elementwise_add_f32(in1, in2, output, -6.0f, 6.0f, 9));
...
void add_f32_nan_inf_arm_elementwise_add_f32(void)
{
    ...
    // no license to fold (#333, #334). Inf + -Inf and a NaN operand must therefore come back as NaN,
    ...
    const float32_t inf = add_f32_from_bits(&inf_bits);
    const float32_t nan = add_f32_from_bits(&nan_bits);
    const float32_t in1[4] = {inf, nan, inf, -inf};
    const float32_t in2[4] = {-inf, 0.0f, 1.0f, 1.0f};
    float32_t output[4] = {0};

    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_SUCCESS, arm_elementwise_add_f32(in1, in2, output, -6.0f, 6.0f, 4));

    TEST_ASSERT_TRUE_MESSAGE(add_f32_bits_are_nan(output[0]), "Expected NaN from Inf + -Inf");
    TEST_ASSERT_TRUE_MESSAGE(add_f32_bits_are_nan(output[1]), "Expected NaN from NaN operand");
    TEST_ASSERT_EQUAL_FLOAT(6.0f, output[2]);
    TEST_ASSERT_EQUAL_FLOAT(-6.0f, output[3]);
}

void add_f32_arg_error_arm_elementwise_add_f32(void)
{
    float32_t in[2] = {0.0f, 0.0f};
    float32_t output[2] = {0};
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR, arm_elementwise_add_f32(NULL, in, output, -6.0f, 6.0f, 2));
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR, arm_elementwise_add_f32(in, in, output, -6.0f, 6.0f, 0));
}
