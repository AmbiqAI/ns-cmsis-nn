...
void minimum_broadcast_row_scalar_s8(void)
{
    const int8_t input_1[3] = {10, 20, 30};
    const int8_t input_2[12] = {1, 2, 3, 4, 11, 12, 13, 14, 21, 22, 23, 24};
    const int8_t expected[12] = {1, 2, 3, 4, 11, 12, 13, 14, 21, 22, 23, 24};
    int8_t output[12] = {0};
    ...
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_SUCCESS,
                      arm_minimum_s8(&ctx, input_1, &input_1_dims, input_2, &input_2_dims, output, &output_dims));
    TEST_ASSERT_TRUE(validate(output, expected, 12));

    memset(output, 0, sizeof(output));
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_SUCCESS,
                      arm_minimum_s8(&ctx, input_2, &input_2_dims, input_1, &input_1_dims, output, &output_dims));
    TEST_ASSERT_TRUE(validate(output, expected, 12));
}

void minimum_arg_error_s8(void)
{
    int8_t data[12] = {0};
    int8_t output[12] = {0};
    cmsis_nn_context ctx = {NULL, 0};
    ...
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR,
                      arm_minimum_s8(&ctx, data, &dims_3x1, data, &dims_2x4, output, &dims_3x4));
    // the output shape must be the broadcast shape
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR,
                      arm_minimum_s8(&ctx, data, &dims_3x1, data, &dims_3x4, output, &dims_3x1));
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR,
                      arm_minimum_s8(&ctx, NULL, &dims_3x1, data, &dims_3x4, output, &dims_3x4));
}
