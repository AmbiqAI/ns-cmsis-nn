...
void slice_f32_arg_error_arm_strided_slice_f32(void)
{
    float32_t output[SLICE_F32_OUTPUT_SIZE] = {0};
    ...
    // A negative output dim once wrapped to a huge unsigned memcpy count
    const cmsis_nn_dims neg_output_dims = {-1, SLICE_F32_OUTPUT_H, SLICE_F32_OUTPUT_W, SLICE_F32_OUTPUT_C};
    const cmsis_nn_dims zero_output_dims = {SLICE_F32_OUTPUT_N, SLICE_F32_OUTPUT_H, SLICE_F32_OUTPUT_W, 0};
    const cmsis_nn_dims zero_stride_dims = {SLICE_F32_STRIDES_N, 0, SLICE_F32_STRIDES_W, SLICE_F32_STRIDES_C};

    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR,
                      arm_strided_slice_f32(NULL, output, &input_dims, &begin_dims, &stride_dims, &output_dims));
    TEST_ASSERT_EQUAL(
        ARM_CMSIS_NN_ARG_ERROR,
        arm_strided_slice_f32(slice_f32_input, output, &input_dims, &begin_dims, &stride_dims, &neg_output_dims));
    TEST_ASSERT_EQUAL(
        ARM_CMSIS_NN_ARG_ERROR,
        arm_strided_slice_f32(slice_f32_input, output, &input_dims, &begin_dims, &stride_dims, &zero_output_dims));
    TEST_ASSERT_EQUAL(
        ARM_CMSIS_NN_ARG_ERROR,
        arm_strided_slice_f32(slice_f32_input, output, &input_dims, &begin_dims, &zero_stride_dims, &output_dims));
}
