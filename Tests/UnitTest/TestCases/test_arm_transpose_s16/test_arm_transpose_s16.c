...
void transpose_dims_mismatch_arm_transpose_s16(void)
{
    const int32_t buffer_size = 1 * 8 * 10 * 12;
    const int16_t poison = (int16_t)0x5a5a;
    static int16_t input_data[1 * 8 * 10 * 12];
    static int16_t output_data[1 * 8 * 10 * 12];
...
// see AmbiqAI/ns-cmsis-nn#443
void transpose_2dim_identity_arm_transpose_s16(void)
{
    const int16_t input_data[6] = {1, 2, 3, 4, 5, 6};
    int16_t output_data[6] = {0};
    ...
    TEST_ASSERT_TRUE(validate_s16(output_data, input_data, 6));
}

// A perm with a repeated axis is not a permutation; the dims agree, so only the repeat can reject it.
// see AmbiqAI/ns-cmsis-nn#443
void transpose_perm_duplicate_axis_arm_transpose_s16(void)
{
    const int32_t input_size = 2 * 2 * 3 * 4;
    const int32_t output_size = 2 * 2 * 2 * 3;
    const int16_t poison = (int16_t)0x5a5a;
    static int16_t input_data[2 * 2 * 3 * 4];
    static int16_t output_data[2 * 2 * 2 * 3];
...
void transpose_num_dims_out_of_range_arm_transpose_s16(void)
{
    const int32_t buffer_size = 2 * 3 * 4 * 5;
    const int16_t poison = (int16_t)0x5a5a;
    static int16_t input_data[2 * 3 * 4 * 5];
    static int16_t output_data[2 * 3 * 4 * 5];
    ...
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR,
                      arm_transpose_s16(input_data, output_data, &input_dims, &output_dims, &too_few));
    TEST_ASSERT_EQUAL(ARM_CMSIS_NN_ARG_ERROR,
                      arm_transpose_s16(input_data, output_data, &input_dims, &output_dims, &too_many));
