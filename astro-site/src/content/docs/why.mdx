...
<CardGrid columns={2}>
  <Card>
    <CardHeader titleAs="h3" eyebrow="Integration"><span>One kernel package, several entry points</span></CardHeader>
    <CardContent>Use the same release through CMake, CMSIS-Pack, Zephyr, or neuralSPOT-X instead of maintaining separate downstream copies.</CardContent>
  </Card>
  <Card>
    <CardHeader titleAs="h3" eyebrow="Acceleration"><span>MVE and DSP stay part of the plan</span></CardHeader>
    <CardContent>Cortex-M55 MVE paths are a primary target, while DSP variants remain important for Apollo-class devices without MVE.</CardContent>
  </Card>
  <Card>
    <CardHeader titleAs="h3" eyebrow="Release control"><span>Artifacts that can be pinned</span></CardHeader>
    <CardContent>Versioned releases, checksums, CMSIS-Pack metadata, and prebuilt archives give firmware teams something concrete to qualify.</CardContent>
  </Card>
  <Card>
    <CardHeader titleAs="h3" eyebrow="HELIA fit"><span>Built for runtime and compiler paths</span></CardHeader>
    <CardContent>heliaRT, heliaAOT, and neuralSPOT-X can all consume the same kernel layer instead of treating kernels as local project plumbing.</CardContent>
  </Card>
</CardGrid>
