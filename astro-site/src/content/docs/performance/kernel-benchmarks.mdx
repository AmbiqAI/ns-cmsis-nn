...
description: Measured cycle counts for the REF, DSP, and MVE paths on Apollo510.
---
...
heliaCORE’s kernel mix delivers large gains on real hardware. These summary
figures reflect the peak average-cycle speedups across the benchmarked kernels
below.
...
<Chart
  title="REF speedup: convolution and matrix-multiply (GCC)"
  subtitle="Speedup against the REF path, by kernel"
  kind="bar"
  orientation="horizontal"
  data={refSpeedups(CONVOLUTION_KERNELS)}
  x="kernel"
  y="speedup"
  series="path"
  scale={{ type: 'log', min: 0.5, max: 16 }}
  axis={{ valueLabel: 'Speedup against REF', categoryLabel: 'Kernel' }}
  valueLabels
  valueFormat={(value) => `${value.toFixed(1)}x`}
  referenceLines={[{ value: 1, label: 'REF baseline' }]}
  width={1220}
  height={520}
/>

Each bar reports speedup relative to REF; values greater than 1.0x indicate lower average cycle count than REF.
...
<Chart
  title="REF speedup: elementwise and scalar (GCC)"
  subtitle="Speedup against the REF path, by kernel"
  kind="bar"
  orientation="horizontal"
  data={refSpeedups(ELEMENTWISE_KERNELS)}
  x="kernel"
  y="speedup"
  series="path"
  scale={{ type: 'log', min: 0.5, max: 16 }}
...
The board, clock, timer, toolchain and iteration count these numbers were taken
under are on [Methodology](../methodology/).
