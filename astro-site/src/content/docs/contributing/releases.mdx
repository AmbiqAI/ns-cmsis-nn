...
[Contributing → Release recovery](../#release-recovery) for
...
- Maintainer release notes in [Contributing](../#maintainer-release-notes)
   explain how to recover when the pipeline fails.
- Toolchain identity is recorded in `manifest.json` inside
  each tarball. See [Toolchain Pinning](../../getting-started/toolchains/).
