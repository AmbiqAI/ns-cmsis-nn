...
/**
 * @brief Weights and biases for a single float32 GRU gate.
 *
...
typedef struct
{
    const float32_t *input_weights;  /**< Input-to-gate weight matrix [hidden_size, input_size]. */
    const float32_t *hidden_weights; /**< Hidden-to-gate weight matrix [hidden_size, hidden_size]. */
    const float32_t *input_bias;     /**< Optional input-projection bias [hidden_size]. May be NULL. */
    const float32_t *hidden_bias;    /**< Optional recurrent-projection bias [hidden_size]. May be NULL. */
} cmsis_nn_gru_gate_f32;

/**
 * @brief Parameters for a float32 unidirectional GRU invocation.
 *
...
typedef struct
{
    ...
    cmsis_nn_gru_gate_f32 update_gate;    /**< Update gate (z), sigmoid activation. */
    cmsis_nn_gru_gate_f32 reset_gate;     /**< Reset gate (r), sigmoid activation. */
    cmsis_nn_gru_gate_f32 candidate_gate; /**< Candidate/new gate (n), tanh activation. */
